...
(function attachEditorTutorial() {
  ...
  const STEPS = [
    ...
    },
    {
      ...
      selectors: ['.choice-quest-fields', '.scene-wizards-bar', 'button[onclick*="openScenarioWizard(\'quest\')"]'],
      prepare(t) {
        t.switchToScenesTab();
        if (Editor.currentScene && typeof Editor.selectScene === 'function') {
          Editor.selectScene(Editor.currentScene);
...
this._wrap('openCreateSceneModal', () => {
      ...
      if (Editor.hooks?.after) {
        ...
        console.warn('[editor-tutorial] Editor.hooks missing — tutorial lifecycle hooks skipped');
      }
    },
