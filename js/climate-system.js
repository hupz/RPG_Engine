...
(function () {
  ...
  function installClimateHooks() {
    ...
    Object.assign(GameEngine, {
      ...
      updateWorldStatusPanelVisibility() {
        const panel = document.getElementById('world-status-panel');
        if (!panel) return;
        const timeOn = !!this.timeSystem?.isEnabled?.();
        const climateOn = !!this.isClimateEnabled?.();
        panel.classList.toggle('hidden', !(timeOn || climateOn));
        const climateRow = document.getElementById('world-status-climate-row');
        climateRow?.classList.toggle('hidden', !climateOn);
      },

      updateMobileWorldStatusCompact() {
        const el = document.getElementById('mobile-compact-world');
        if (!el) return;
        const parts = [];
        if (this.timeSystem?.isEnabled?.()) {
          parts.push(this.timeSystem.getTimeString());
          const day = this.timeSystem.state?.day;
          if (day != null) parts.push(`Д${day}`);
        }
        const dateStr = this.seasonSystem?.getDateString?.() || '';
        if (dateStr) parts.push(dateStr);
        const wIcon = this.weatherSystem?.WEATHER_TYPES?.[this.weatherSystem?.state?.current]?.icon;
        if (this.isClimateEnabled?.() && wIcon) parts.push(wIcon);
      ...
      updateClimateUI() {
        ...
        const dateStr = this.seasonSystem?.getDateString?.() || '';
        set('date-display', dateStr);
        const dateSep = document.querySelector('.world-status-sep--date');
        const timeOn = !!this.timeSystem?.isEnabled?.();
        dateSep?.classList.toggle('hidden', !dateStr || !timeOn);
        document.getElementById('date-display')?.classList.toggle('hidden', !dateStr);
        ...
        this.updateWorldStatusPanelVisibility();
        this.timeSystem?.updateUI?.();

        const effectsEl = document.getElementById('climate-effects');
        if (effectsEl && !climateOn) effectsEl.innerHTML = '';
      ...
      applyClimateSpellMods(baseDmg, damageType) {
        ...
        const map = { fire: 'fire', cold: 'cold', ice: 'cold', lightning: 'electric', thunder: 'thunder', acid: 'acid', electric: 'electric', magical: 'arcane', darkness: 'dark', curse: 'curse' };
