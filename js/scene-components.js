...
const SceneComponentRegistry = (function () {
  ...
  const COMPONENT_META = {
    ...
    upgrade: { label: 'Улучшение', icon: '⬆️' },
    enhance: { label: 'Улучшение', icon: '⬆️' },
    heal: { label: 'Лечение', icon: '🩹' },
    curse_remove: { label: 'Снятие проклятий', icon: '✨' },
    gambling: { label: 'Азартные игры', icon: '🎲' },
    crafting: { label: 'Создание предметов', icon: '🔨' },
    interactive: { label: 'Кнопка (цепочка)', icon: '🔘' }
  };

  function register(type, renderer) {
    _types[type] = renderer;
    if (type === 'enhance') _types.upgrade = renderer;
  }
