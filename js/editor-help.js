...
(function attachEditorHelp() {
  ...
  /**
   * insertBefore requires referenceNode to be a *direct child* of parent.
   * querySelector may return a nested descendant → DOMException.
   * Walk up until we find an ancestor that is a child of parent.
   */
  function insertBeforeSafe(parent, newNode, reference) {
    if (!parent || !newNode) return;
    if (!parent.isConnected && parent.nodeType !== 1) return;
    let ref = reference || null;
    while (ref && ref.parentNode !== parent) {
      ref = ref.parentNode;
    }
  ...
  function enhanceFormGroup(group) {
    ...
    // Skip nodes already detached from the live document (stale after rerender)
    if (group.ownerDocument && group.ownerDocument.body && !group.isConnected) return;
    ...
    } else if (!group.querySelector('.editor-help-orphan-row')) {
      const row = document.createElement('div');
      row.className = 'editor-help-orphan-row';
      row.appendChild(trigger);
      const first = group.firstElementChild;
      insertBeforeSafe(group, row, first);
    }

    if (!group.querySelector('.editor-help-inline')) {
      const anchor = group.querySelector('.hint')
        || group.querySelector('input, select, textarea, .grid-2, .icon-picker-row');
      insertBeforeSafe(group, inline, anchor);
    }
  }

  function enhanceConditionBuilder(builder) {
    if (!builder || builder.nodeType !== 1) return;
    if (builder.dataset.helpEnhanced === '1') return;
    if (builder.ownerDocument && builder.ownerDocument.body && !builder.isConnected) return;
    ...
    if (!builder.querySelector('.editor-help-inline')) {
      ...
      // Prefer insert after head/body if they are direct children; else append
      const ref = (body && body.parentNode === builder) ? body.nextSibling : null;
      if (ref && ref.parentNode === builder) {
        builder.insertBefore(inline, ref);
      } else if (body && body.parentNode === builder) {
        // after body
        if (body.nextSibling) builder.insertBefore(inline, body.nextSibling);
        else builder.appendChild(inline);
      } else {
        builder.appendChild(inline);
      }
    }
  }

  function enhance(root) {
    const container = root && root.querySelectorAll
      ? root
      : (typeof document !== 'undefined' && (document.querySelector('.main-area') || document));
    if (!container || !container.querySelectorAll) return;
    // container may be a detached tab fragment — still enhance its subtree
    ...
    if (typeof document !== 'undefined' && document.body?.classList.contains('editor-help-show-all')) {
      hideTooltip(true);
    }
  }
  ...
  function scheduleEnhance() {
    ...
    enhanceTimer = setTimeout(() => {
      ...
      // If active tab was removed between schedule and fire — enhance main area / document safely
      if (active && active.isConnected === false) {
        enhance(document.querySelector('.main-area') || document);
        return;
      }
  ...
  if (typeof Editor !== 'undefined') {
    ...
    // Prefer hooks.after — never nest Editor.renderAll() inside a renderAll wrapper
    if (Editor.hooks && typeof Editor.hooks.after === 'function') {
      helpMethods.forEach((name) => {
