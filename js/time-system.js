  ...
  const TimeSystem = {
    ...
    advance(gameMinutes) {
      ...
      if (typeof QuestEvents !== 'undefined') {
        QuestEvents.emit('TimePassed', {
          minutes: mins,
          hour: this.state.hour,
          day: this.state.day,
          totalGameHours: this.state.totalGameHours
        });
      }
      this.updateUI();
    },
    ...
    updateUI() {
      ...
      const timeDaySep = document.querySelector(
        '.world-status-line--datetime .world-status-sep:not(.world-status-sep--date)'
      );
      const icon = document.getElementById('time-icon');
      const datetimeLine = document.querySelector('.world-status-line--datetime');

      const timeOn = this.isEnabled();
      const dateStr = this.engine?.seasonSystem?.getDateString?.() || '';

      if (timeDisplay) {
        timeDisplay.textContent = timeOn ? this.getTimeString() : '';
        timeDisplay.classList.toggle('hidden', !timeOn);
      }
      if (dayDisplay) {
        dayDisplay.textContent = timeOn ? `День ${this.state.day}` : '';
        dayDisplay.classList.toggle('hidden', !timeOn);
      }
      if (dateDisplay) {
        dateDisplay.textContent = dateStr;
        dateDisplay.classList.toggle('hidden', !dateStr);
      }
      if (dateSep) dateSep.classList.toggle('hidden', !dateStr || !timeOn);
      if (timeDaySep) timeDaySep.classList.toggle('hidden', !timeOn);
      if (datetimeLine) {
        const showDatetime = timeOn || !!dateStr;
        datetimeLine.classList.toggle('hidden', !showDatetime);
      }
      ...
      if (icon) {
        ...
        icon.classList.toggle('hidden', !timeOn);
      }

      if (panel && timeOn) {
        panel.classList.remove('period-dawn', 'period-day', 'period-dusk', 'period-night');
        panel.classList.add(`period-${period}`);
      }

      this.engine?.updateWorldStatusPanelVisibility?.();
      this.engine?.updateMobileWorldStatusCompact?.();
    },
