...
function syncCharacterToUI(engine, character) {
  ...
  // Quest v2: no QuestRuntime.onCharacterCreated — progress via QuestRuntime events
  if (typeof QuestRuntime !== 'undefined' && typeof QuestRuntime.onCharacterCreated === 'function') {
    QuestRuntime.onCharacterCreated(engine, c);
  }
}
    ...
    return {
      ...
      class: cls ? {
        ...
        abilities: (cls.abilities || []).map((a) => {
          ...
          const pool = engine.data?.progression?.abilities || {};
          const def = (id && pool[id]) || (typeof a === 'object' ? a : null) || {};
          return {
            id: id || def.id,
            name: def.name || id,
