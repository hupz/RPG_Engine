...
(function attachNlConditionBridge() {
  ...
  function isNlRuleType(type) {
    return NLConditionBuilder.isSupportedRuleType(type);
  }
  ...
  Editor.renderRuleRow = function (rule, ruleIndex, builderId) {
    ...
    let html = origRenderRuleRow(rule, ruleIndex, builderId);
    if (isNlRuleType(type)) {
      html = html.replace('cb-rule-row flex-row', 'cb-rule-row cb-rule-row--nl flex-row');
      html = html.replace(
        'data-rule-index="' + ruleIndex + '"',
        'data-rule-index="' + ruleIndex + '" data-nl-rule-type="' + type + '"'
      );
    }
    return html;
  };
  ...
  Editor.renderRuleFields = function (rule, ruleIndex) {
    ...
    const catalog = (type === 'flag' || type === 'notFlag')
      ? NLConditionBuilder.getFlagCatalog(this.data, this.getFlagCatalog?.())
      : null;
  ...
  function updateNlPhrasePreview(row, rule, gameData) {
    const phraseEl = row?.querySelector('.nl-condition-phrase');
    if (!phraseEl || !rule) return;
    phraseEl.textContent = NLConditionBuilder.ruleToPhrase(rule, gameData);
  }

  function updateNlJsonPreview(row, rule) {
    const preview = row?.querySelector('[data-nl-preview]');
    if (!preview) return;
    preview.textContent = rule ? JSON.stringify(rule) : '{}';
  }
  ...
  function syncQuestStageOptions(row, editor) {
    const type = row.dataset.nlRuleType;
    if (type !== 'questStage') return;
    const questSel = row.querySelector('[data-nl-action="questId"]');
    const stageSel = row.querySelector('[data-nl-action="stage"]');
    if (!questSel || !stageSel) return;
    const qid = questSel.value;
    const cur = stageSel.value;
    stageSel.innerHTML = NLConditionBuilder.questStageSelectHtml(qid, cur, editor);
  }
  ...
  Editor.handleNlConditionChange = function (nlEl) {
    ...
    const type = row.dataset.nlRuleType || NLConditionBuilder.inferRuleType(rule);
    ...
    } else if (type === 'notFlag') {
      const state = NLConditionBuilder.readStateFromRow(row, this.data);
      NLConditionBuilder.applyStateToRule(rule, state);
    } else {
      const state = NLConditionBuilder.readStateFromRow(row, this.data);
      NLConditionBuilder.applyStateToRule(rule, state);
      if (nlEl.dataset.nlAction === 'questId' && type === 'questStage') {
        syncQuestStageOptions(row, this);
        needsRefresh = true;
      }
    }

    updateNlJsonPreview(row, rule);
    updateNlPhrasePreview(row, rule, this.data);
  ...
  Editor.ensureConditionBuilderEvents = function () {
    ...
    document.addEventListener('input', (e) => {
      if (e.target.matches?.('[data-nl-action]')) {
