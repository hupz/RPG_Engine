...
(function attachProjectSearchAndHoles() {
  ...
  Object.assign(Editor, {
    ...
    runProjectSearch(query) {
      ...
      const index = typeof Editor.Perf?.getCachedProjectSearchIndex === 'function'
        ? Editor.Perf.getCachedProjectSearchIndex(this.buildProjectSearchIndex)
    ...
    onProjectSearchInput(value) {
      ...
      if (typeof Editor.Perf?.debouncedProjectSearchRender === 'function') {
        Editor.Perf.debouncedProjectSearchRender();
        return;
      }
      this._renderProjectSearchResultsInline(value);
    },

    _renderProjectSearchResultsInline(value) {
  ...
  // Hotkey Ctrl+K / Cmd+K — delegates to unified command palette (UI-16)
