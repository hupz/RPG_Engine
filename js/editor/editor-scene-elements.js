...
(function attachEditorSceneElements() {
  ...
  Object.assign(Editor, {
    ...
    async removeSceneElement(elementId) {
      const scene = getScene();
      if (!scene) return;
      ...
      if (idx === -1) return;
      if (!(await Editor.confirmDialog({ message: tr('editor.sceneElements.deleteConfirm'), danger: true }))) return;
      list.splice(idx, 1);
      syncAndPreview();
      this.renderSceneEditor();
    },
    ...
    updateElementDataJson(elementId, jsonStr) {
      ...
      try {
        ...
        Editor.toast.error(tr('editor.forms.jsonError', { message: e.message }));
      }
    },
      ...
      switch (el.type) {
        ...
        case 'combat': {
          const tags = (d.enemies || []).map((eid2, i) =>
            `<div class="enemy-tag">${this.escapeHtml(eid2)}<span class="remove" onclick="${this.escapeAttr('Editor.removeCombatEnemyFromElement(' + JSON.stringify(eid) + ', ' + i + ')')}">×</span></div>`
          ).join('');
    ...
    renderElementCard(el, index, listKey) {
      ...
      return `<div class="scene-element-card project-info" data-element-id="${eid}" style="margin-bottom:10px;">
        <div style="display:flex;justify-content:space-between;align-items:center;gap:8px;">
          <button type="button" class="btn btn-secondary" style="font-size:12px;padding:4px 8px;" onclick="${this.escapeAttr('Editor.toggleElementCollapse(' + JSON.stringify(eid) + ')')}">${collapsed ? '▶' : '▼'}</button>
          <strong style="flex:1;">${this.escapeHtml(title)}</strong>
          <button type="button" class="btn btn-secondary" title="↑" onclick="${this.escapeAttr('Editor.moveSceneElement(' + JSON.stringify(eid) + ', -1)')}">↑</button>
          <button type="button" class="btn btn-secondary" title="↓" onclick="${this.escapeAttr('Editor.moveSceneElement(' + JSON.stringify(eid) + ', 1)')}">↓</button>
          <button type="button" class="btn btn-danger" onclick="${this.escapeAttr('Editor.removeSceneElement(' + JSON.stringify(eid) + ')')}">🗑</button>
        </div>
    ...
    renderSceneElementsSection(scene) {
      ...
      const pickerItems = PICKER_TYPES.map((t) => {
        ...
        return `<button type="button" class="btn btn-secondary scene-element-pick" style="margin:4px;" onclick="${this.escapeAttr('Editor.pickElementType(' + JSON.stringify(t) + ')')}">${this.escapeHtml(this.renderElementTypeLabel(t))}</button>`;
      }).join('');
    ...
    renderSceneAdvancedSection(scene) {
      const npcField = typeof this.renderEntityPicker === 'function'
        ...
        : (() => {
            ...
            return `<select onchange="Editor.setSceneNpcId(this.value)"><option value="">—</option>${npcOpts}</select>`;
          })();
  ...
  function ensureElementsBeforeRender() {
  ...
  function appendSceneElementsPanels() {
    ...
    container.querySelectorAll('.form-group').forEach((fg) => {
      ...
      if (txt.includes('Предметы') || txt.includes('Золото') || txt.includes('Items') || txt.includes('Gold')) {
  ...
  if (Editor.hooks && typeof Editor.hooks.before === 'function') {
    Editor.hooks.before('renderSceneEditor', function (args) {
      ensureElementsBeforeRender.call(this);
      return args;
    });
    Editor.hooks.after('renderSceneEditor', function () {
      appendSceneElementsPanels.call(this);
    });
  }

  if (Editor.hooks && typeof Editor.hooks.after === 'function') {
    Editor.hooks.after('selectScene', function (result, args) {
      const id = args && args[0];
      const scene = this.data?.scenes?.[id];
      if (scene && this._useSceneElements) ensureSceneElements(scene);
      return result;
    });
    Editor.hooks.before('updateJSONPreview', function (args) {
    ...
    if (typeof Editor.applyLoadedProject === 'function') {
      Editor.hooks.after('applyLoadedProject', function (result) {
        if (this._useSceneElements) this.migrateAllSceneElements();
        return result;
      });
    }
  } else if (typeof console !== 'undefined' && console.warn) {
    console.warn('[editor-scene-elements] Editor.hooks missing — extension skipped');
  }
})();
