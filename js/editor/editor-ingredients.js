...
(function attachEditorIngredients() {
  ...
  Object.assign(Editor, {
    ...
    async createIngredient() {
      this.ensureCraftingData();
      const name = await Editor.promptDialog({ message: 'Название ингредиента:', defaultValue: 'Новый ингредиент' });
      if (!name?.trim()) return;
    ...
    saveIngredient(id) {
      ...
      if (!ing) return;
      if (!ing.name?.trim()) {
        Editor.toast.warning('Укажите название ингредиента.');
        return;
      }
      ing.name = ing.name.trim();
      this.syncIngredientToItem(ing);
      this.updateJSONPreview();
      Editor.toast.success('✅ Ингредиент сохранён');
      this.renderIngredients();
    },
    ...
    async deleteIngredient(id) {
      ...
      if (this.isIngredientUsedInRecipes(id)) {
        Editor.toast.warning('Нельзя удалить: ингредиент используется в рецептах.');
        return;
      }
      if (!(await Editor.confirmDialog({ message: `Удалить ингредиент «${this.data.ingredients[id].name || id}»?`, danger: true }))) return;
    ...
    importIngredientsJson() {
      ...
      input.onchange = async (e) => {
        ...
        try {
          ...
          this.renderIngredients();
          this.updateJSONPreview();
          Editor.toast.success('✅ Ингредиенты импортированы');
        } catch (err) {
      ...
      return `<div class="quest-detail-card craft-detail-card">
        <div class="quest-detail-head">
          ...
          <div class="craft-detail-actions">
            <button type="button" class="btn btn-primary" onclick="${this.escapeAttr('Editor.saveIngredient(' + JSON.stringify(iid) + ')')}">💾 Сохранить</button>
            <button type="button" class="btn btn-danger" onclick="${this.escapeAttr('Editor.deleteIngredient(' + JSON.stringify(iid) + ')')}">🗑️ Удалить</button>
          </div>
        </div>
    ...
    renderIngredients() {
      ...
      const sidebar = ids.map((id) => {
        ...
        return `<button type="button" class="quest-pick${active}" onclick="${this.escapeAttr('Editor.selectIngredientToEdit(' + JSON.stringify(id) + ')')}">${ing.icon || '🌿'} ${this.escapeHtml(ing.name || id)}</button>`;
      }).join('');
  ...
  // НЕ заменять Editor.renderAll (иначе: renderAll → crafting → hooked → renderAll).
  // ensureCraftingData — before render; без вызова renderAll из migration.
  function ensureCraftingBeforeRender() {
    if (typeof Editor.ensureCraftingData === 'function') {
      Editor.ensureCraftingData();
    }
  }
  if (Editor.hooks && typeof Editor.hooks.before === 'function') {
    Editor.hooks.before('renderAll', function (args) {
      ensureCraftingBeforeRender();
      return args;
    });
  } else if (typeof console !== 'undefined' && console.warn) {
    console.warn('[editor-ingredients] Editor.hooks missing — extension skipped');
  }
})();
