...
(function attachEditorReputation() {
  ...
  Object.assign(Editor, {
    ...
    async createFaction() {
      this.ensureReputationFactions();
      const id = await Editor.promptDialog({ message: 'ID фракции (латиница, напр. rep_bandits):', defaultValue: 'rep_new' });
      if (!id || !/^rep_[a-z][a-z0-9_]*$/i.test(id)) {
        Editor.toast.warning('ID: rep_ + латиница');
        return;
      }
      if (this.data.reputation[id]) {
        Editor.toast.warning('Фракция уже существует');
        return;
      }
      const name = (await Editor.promptDialog({ message: 'Название фракции:', defaultValue: 'Новая фракция' })) || 'Новая фракция';
    ...
    async deleteFaction(id) {
      if (!(await Editor.confirmDialog({ message: 'Удалить фракцию «' + id + '»?', danger: true }))) return;
    ...
    removeFactionLevel(id, idx) {
      ...
      if (!f?.levels || f.levels.length <= 1) {
        Editor.toast.warning('Нужен хотя бы один уровень');
        return;
      }
    ...
    saveFactionForm(id) {
      ...
      this.renderReputation();
      this.updateJSONPreview();
      Editor.toast.success('Фракция сохранена в JSON проекта.');
    },
