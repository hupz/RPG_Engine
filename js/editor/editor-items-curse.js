...
(function attachEditorItemsCurse() {
    ...
    const pills = effects.map(eff => {
      ...
      return `<span class="curse-effect-pill" data-effect="${this.escapeAttr(eff)}">
        ...
        <button type="button" class="curse-pill-remove" title="Удалить"
          onclick="${this.escapeAttr('Editor.removeItemCurseEffect(' + JSON.stringify(itemId) + ', ' + JSON.stringify(eff) + ')')}">❌</button>
      </span>`;
        ...
        <div class="item-curse-fields${cursed ? ' item-curse-fields--open' : ''}" id="curse-fields-${this.escapeAttr(itemId)}">
          <div class="form-group curse-effects-group${effectsErr ? ' field-error' : ''}">
            ...
            <div class="curse-effects-toolbar">
              ...
              <button type="button" class="btn btn-secondary" onclick="${this.escapeAttr('Editor.addItemCurseEffect(' + JSON.stringify(itemId) + ')')}">Добавить эффект</button>
            </div>
  ...
  const _renderItemTypeExtra = function (id, it) {
    if (it.type === 'weapon') {
      const hands = (it.hands || 'one') === 'two' ? 'two' : 'one';
      return `<div class="grid-3">
          ...
          <div class="form-group"><label>Занимает рук</label>
            <select onchange="Editor.updateItemData('${id}','hands',this.value)">
              <option value="one" ${hands === 'one' ? 'selected' : ''}>Одну руку</option>
              <option value="two" ${hands === 'two' ? 'selected' : ''}>Две руки</option>
            </select>
          </div>
  ...
  Editor.renderItemDetail = function (itemId) {
    ...
    if (typeof ItemsRewardsIndex !== 'undefined' && ItemsRewardsIndex.validateItemShape) {
      const v = ItemsRewardsIndex.validateItemShape(it, itemId);
    ...
    const showTopIcon = it.type !== 'accessory';
    return `<div class="quest-editor item-detail-editor">
        <h4>${this.escapeHtml(it.name || 'Без названия')} (${this.escapeHtml(itemId)})</h4>
        ${validationHtml}
        <div class="form-group"><label>ID</label><input value="${this.escapeHtml(itemId)}" disabled></div>
        <div class="form-group"><label>Name</label><input value="${this.escapeHtml(it.name || '')}" onchange="Editor.updateItemData('${itemId}','name',this.value)"></div>
        <div class="form-group"><label>Category</label><select onchange="Editor.updateItemData('${itemId}','type',this.value)">${typeOpts}</select></div>
        <div class="form-group"><label>Description</label><textarea onchange="Editor.updateItemData('${itemId}','description',this.value)">${this.escapeHtml(descVal)}</textarea></div>
        ${showTopIcon ? `<div class="form-group"><label>Icon</label><input value="${this.escapeHtml(it.icon || '')}" placeholder="emoji or asset id" onchange="Editor.updateItemData('${itemId}','icon',this.value)"></div>` : ''}
        <div class="grid-2">
          <div class="form-group"><label><input type="checkbox" ${stackable ? 'checked' : ''} onchange="Editor.updateItemData('${itemId}','stackable',this.checked)"> Stackable</label></div>
          <div class="form-group"><label>Max stack</label>
            <input type="number" min="1" value="${this.escapeHtml(String(maxStack))}" ${stackable ? '' : 'disabled'}
              onchange="Editor.updateItemData('${itemId}','maxStack',parseInt(this.value,10)||1)"></div>
        </div>
        ${extra}
        ${this.renderItemCurseSection(itemId, it)}
        <button class="btn btn-danger" onclick="${this.escapeAttr('Editor.deleteItem(' + JSON.stringify(itemId) + ')')}">🗑 Удалить</button>
      </div>`;
  };
  ...
  Editor.renderItems = function () {
    ...
    const sidebar = ids.map(id => {
      ...
      const curseIcon = it.cursed === true
        ...
        : '';
      const handsIcon = it.type === 'weapon'
        ? `<span class="item-pick-hands" title="${(it.hands || 'one') === 'two' ? 'Двуручное' : 'Одноручное'}">${(it.hands || 'one') === 'two' ? '⚔️' : '🗡️'}</span>`
        : '';
      const effectsTip = it.cursed === true ? this.formatItemCurseTooltip(it) : '';
      return `<button type="button" class="class-pick item-pick ${active}" onclick="${this.escapeAttr('Editor.selectItemToEdit(' + JSON.stringify(id) + ')')}"
        title="${it.cursed ? 'Проклят: ' + this.escapeAttr(effectsTip) : ''}">
        ${curseIcon}${handsIcon} ${this.escapeHtml(it.name || id)}
      </button>`;
    }).join('');
  ...
  Editor.createItem = async function () {
