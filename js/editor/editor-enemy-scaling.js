...
(function attachEditorEnemyScaling() {
  ...
  Object.assign(Editor, {
    ...
    updateScalingLevelKey(idx, oldLevel, newLevel) {
      ...
      if (levels.includes(newLevel) && newLevel !== oldLevel) {
        Editor.toast.warning('Такой уровень уже есть в таблице');
        this.renderBalance();
        return;
      }
    ...
    addScalingLevel() {
      ...
      if (next > 20) {
        Editor.toast.warning('Максимум 20 уровней в таблице');
        return;
      }
    ...
    removeScalingLevel(level) {
      ...
      const levels = this.getScalingLevelList(es);
      if (levels.length <= 1) {
        Editor.toast.warning('Нужна хотя бы одна строка в таблице');
        return;
      }
    ...
    async resetEnemyScalingDefaults() {
      if (!(await Editor.confirmDialog({ message: 'Сбросить таблицу масштабирования к значениям по умолчанию?' }))) return;
  ...
  function appendEnemyScalingHint() {
    ...
    panel.innerHTML = `<div class="project-info hint-box">
        ...
        <button type="button" class="btn btn-secondary" data-action="open-balance">Открыть «Баланс»</button>
      </div>`;
    panel.querySelector('[data-action="open-balance"]')?.addEventListener('click', (e) => {
      e.preventDefault();
      Editor.switchTab?.('balance', e);
    });
  }
  if (Editor.hooks && typeof Editor.hooks.after === 'function') {
    Editor.hooks.after('renderProgression', function () {
      appendEnemyScalingHint();
    });
  } else if (typeof Editor.renderProgression === 'function') {
