  ...
  Object.assign(Editor, {
    ...
    renderClassPresetsBar() {
      ...
      return `<div class="class-presets-bar project-info">
        ...
        <p class="hint">Один клик: готовый класс. Умения попадут в общий пул и назначатся классу.</p>
    ...
    applyClassPreset(presetId) {
      try {
        ...
        if (!preset) {
          Editor.toast.warning('Пресет не найден: ' + presetId);
          return;
        }
        if (!this.data) {
          Editor.toast.warning('Сначала загрузите или создайте проект (вкладка проекта / «Новая история»).');
          return;
        }
        ...
        // Умения пресета → общий пул; у класса только id
        body.abilities = body.abilities.map((ab, i) => {
          if (typeof ab === 'string') return ab;
          if (!ab || typeof ab !== 'object') return null;
          const aid = ab.id || (baseId + '_ab' + i);
          const copy = Object.assign({}, ab, { id: aid });
          return typeof this.upsertAbilityToPool === 'function'
            ? this.upsertAbilityToPool(copy)
            : aid;
        }).filter(Boolean);
        ...
        if (ok) {
          Editor.toast.success('Класс «' + preset.name + '» добавлен. Выберите его слева, если список уже открыт.');
        } else {
          Editor.toast.error('Класс не сохранился в data.classes — проверьте консоль.');
        }
      } catch (err) {
        console.error('applyClassPreset', err);
        Editor.toast.error('Не удалось добавить класс: ' + (err && err.message ? err.message : err));
      }
    },
    ...
    renderAbilityWizardPanel(classId) {
      if (!this._abilityWizardOpen) {
        return `<div class="ability-wizard-bar">
          ...
          <span class="hint">Создаёт умение в общем пуле и назначает текущему классу.</span>
        </div>`;
      }
      ...
      const dmgTypes = this.ABILITY_DAMAGE_TYPES || {
        physical: { label: 'Физический', icon: '⚔️' },
        magical: { label: 'Магический', icon: '✨' },
        fire: { label: 'Огненный', icon: '🔥' },
        heal: { label: 'Лечение', icon: '💚' }
      };
      const dmgTypeOpts = Object.entries(dmgTypes).map(([k, m]) =>
        `<option value="${k}">${m.icon || ''} ${this.escapeHtml(m.label)}</option>`
      ).join('');
      const countOpts = (this.DICE_COUNT_OPTIONS || [1, 2, 3, 4, 5, 6]).map((n) =>
        `<option value="${n}" ${n === 1 ? 'selected' : ''}>${n}</option>`
      ).join('');
      const dieOpts = (this.DICE_SIDE_OPTIONS || ['d4', 'd6', 'd8', 'd10', 'd12', 'd20']).map((d) =>
        `<option value="${d}" ${d === 'd6' ? 'selected' : ''}>${d.toUpperCase()}</option>`
      ).join('');
      return `<div class="ability-wizard-panel project-info">
        ...
        <div class="form-group"><label>Тип урона / эффекта</label>
          <select id="wiz-ab-damage-type">${dmgTypeOpts}</select></div>
        <div class="form-group"><label>Кубики</label>
          <div class="grid-3" style="align-items:end;">
            <div class="form-group" style="margin:0;"><label class="hint">Сколько</label>
              <select id="wiz-ab-dice-count">${countOpts}</select>
            </div>
            <div class="form-group" style="margin:0;"><label class="hint">Кубик</label>
              <select id="wiz-ab-dice-die">${dieOpts}</select>
            </div>
            <div class="form-group" style="margin:0;"><label class="hint">Бонус</label>
              <input type="number" id="wiz-ab-dice-bonus" value="0" step="1" style="max-width:80px"></div>
          </div></div>
    ...
    applyAbilityWizard(classId) {
      ...
      if (!cls) {
        Editor.toast.warning('Сначала создайте или выберите класс (пресет или «Пустой класс»).');
        return;
      }
      classId = cid;
      ...
      const damageTypeKey = document.getElementById('wiz-ab-damage-type')?.value || 'physical';
      const diceCount = parseInt(document.getElementById('wiz-ab-dice-count')?.value, 10) || 1;
      const diceDie = document.getElementById('wiz-ab-dice-die')?.value || 'd6';
      const diceBonus = parseInt(document.getElementById('wiz-ab-dice-bonus')?.value, 10) || 0;
      const dice = typeof Editor.buildDiceFormula === 'function'
        ? Editor.buildDiceFormula(diceCount, diceDie, diceBonus)
        : (diceCount + diceDie.replace('d', 'd') + (diceBonus ? (diceBonus > 0 ? '+' + diceBonus : diceBonus) : ''));
      ...
      if (typeof this.normalizeClassAbilityRefs === 'function') {
        this.normalizeClassAbilityRefs(classId);
      }
      const pool = typeof this.ensureAbilityPool === 'function' ? this.ensureAbilityPool() : (this.data.progression?.abilities || {});
      const abId = (typeof this.slugifyId === 'function'
        ? this.slugifyId(name, classId, pool || {})
        : classId + '_ab_' + (cls.abilities.length + 1));

      let effect = { type: kind };
      if (kind === 'damage') {
        effect = { type: 'damage', value: dice, damageType: damageTypeKey === 'heal' ? 'physical' : damageTypeKey };
      } else if (kind === 'heal') {
      ...
      if (typeof Editor.applyEditorDamageTypeToEffect === 'function' && damageTypeKey) {
        Editor.applyEditorDamageTypeToEffect(effect, damageTypeKey);
        if (effect.type === 'damage' || effect.type === 'heal') effect.value = dice;
      }
      ...
      const ability = {
        ...
        combatOnly,
        oncePerCombat: false,
        usage: combatOnly ? 'combat' : 'both',
        type: 'active',
        effect
      };
      const id = typeof this.upsertAbilityToPool === 'function'
        ? this.upsertAbilityToPool(ability)
        : abId;
      if (id && !cls.abilities.includes(id)) cls.abilities.push(id);
  ...
  if (!window._classPresetClickBound) {
    ...
    document.addEventListener('click', (e) => {
      ...
      if (id && typeof Editor.applyClassPreset === 'function') {
        ...
        Editor.toast.error('applyClassPreset недоступен — перезагрузите редактор.');
      }
    });
  }
