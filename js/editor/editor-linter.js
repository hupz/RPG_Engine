...
(function attachEditorLinter() {
  ...
  Object.assign(Editor, {
    ...
    validateProject() {
      ...
      const issues = typeof ValidateScenes !== 'undefined'
        ? ValidateScenes.validate(this.data)
        : [];
      ...
      const brokenLinks = issues
        ...
        .map((e) => ({ fromScene: e.sceneId, field: e.field, targetId: e.targetId }));
    ...
    showValidationModal(result) {
      ...
      if (!errors.length && !deadEnds.length && !warnings.length) {
        ...
        const issueJump = (issue) => {
          if (typeof Editor.navigateToValidationIssue === 'function') {
            return `Editor.closeValidationModal();Editor.navigateToValidationIssue(${JSON.stringify(issue)})`;
          }
          if (issue.sceneId) {
            return `Editor.closeValidationModal();Editor.openSceneFromGraph(${JSON.stringify(issue.sceneId)})`;
          }
          return 'Editor.closeValidationModal()';
        };

        if (errors.length) {
          html += `<section class="editor-validation-section"><h3>❌ Ошибки (${errors.length})</h3><ul class="editor-validation-list">`;
          html += errors.map((e) =>
            `<li><button type="button" class="editor-validation-link" onclick="${issueJump(e)}">${this.escapeHtml(e.message)}</button></li>`
          ).join('');
          html += '</ul></section>';
        }

        if (deadEnds.length) {
          html += `<section class="editor-validation-section"><h3>⚠ Тупики (${deadEnds.length})</h3><ul class="editor-validation-list">`;
          html += deadEnds.map((e) =>
            `<li><button type="button" class="editor-validation-link" onclick="${issueJump(e)}">${this.escapeHtml(e.message)}</button></li>`
          ).join('');
          html += '</ul></section>';
        ...
        if (warnings.length) {
          html += `<section class="editor-validation-section"><h3>⚠ Предупреждения (${warnings.length})</h3><ul class="editor-validation-list">`;
          html += warnings.map((e) =>
            `<li><button type="button" class="editor-validation-link" onclick="${issueJump(e)}">${this.escapeHtml(e.message)}</button></li>`
          ).join('');
          html += '</ul></section>';
