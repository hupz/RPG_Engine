...
(function attachEditorClimate() {
  ...
  const DAY_LENGTH_LABELS = {
    dawn: 'Рассвет (ч)',
    dayStart: 'Начало дня (ч)',
    dusk: 'Сумерки (ч)',
    night: 'Ночь (ч)'
  };
  ...
  const WEATHER_LABELS = {
    ...
    storm: '🌪️ Шторм',
    ice: '🧊 Гололёд',
    light_rain: '🌦️ Небольшой дождь',
    heavy_rain: '⛈️ Сильный дождь',
    light_snow: '🌨️ Небольшой снег'
  };
  const WEATHER_VARIANT_KEYS = ['rain', 'light_rain', 'heavy_rain', 'snow', 'light_snow', 'thunderstorm', 'blizzard', 'fog'];

  function weatherLabel(id) {
    if (WEATHER_LABELS[id]) return WEATHER_LABELS[id];
    const fromData = typeof ClimateData !== 'undefined' && ClimateData.DEFAULT_WEATHER_TYPES?.[id];
    if (fromData?.name) return (fromData.icon ? fromData.icon + ' ' : '') + fromData.name;
    return id;
  }
  ...
  Editor.renderClimateTab = function () {
    ...
    el.innerHTML = `<div class="climate-editor-wrap">
        ...
        ${SEASON_IDS.map(sid => {
          ...
          return `<div class="season-daylen" data-season="${sid}">
            ...
            <div class="grid-4" style="margin-top:6px;">
              ${['dawn','dayStart','dusk','night'].map(k => `<label>${DAY_LENGTH_LABELS[k] || k}: <input type="number" min="0" max="23" value="${d[k]}"
      ...
      <div class="climate-section project-info">
        ...
        <p class="hint">Числа — вероятность погоды в % (например, «Дождь: 25» = 25% шанс дождя в этот сезон).</p>
  ...
  Editor.renderRegionEditor = function (regionId, region) {
    ...
    return `<div class="region-weather project-info" style="margin:10px 0;padding:10px;border:1px solid var(--border);">
      <div style="display:flex;justify-content:space-between;align-items:center;">
        ...
        ${regionId !== 'default' ? `<button type="button" class="btn btn-danger" style="font-size:11px;" onclick="${this.escapeAttr('Editor.removeClimateRegion(' + JSON.stringify(regionId) + ')')}">×</button>` : ''}
      </div>
      ...
      ${SEASON_IDS.map(sid => {
        ...
        return `<div class="season-weights" style="margin-top:8px;">
          <strong>${SEASON_LABELS[sid]}</strong>
          <div class="weather-weight-grid" style="display:flex;flex-wrap:wrap;gap:6px 12px;margin-top:6px;">
          ${WEATHER_KEYS.map(wk => `<label style="display:inline-flex;align-items:center;gap:4px;font-size:13px;" title="Вероятность: ${weatherLabel(wk)}">
            ${weatherLabel(wk)}:
            <input type="number" class="w-${wk}" value="${sw[wk] ?? 0}" min="0" max="100" style="width:48px;"
  ...
  Editor.addClimateRegion = async function () {
    const id = await Editor.promptDialog({ message: 'ID региона (латиница):', defaultValue: 'mountains' });
    if (!id || !/^[a-z0-9_]+$/i.test(id)) return;
    const c = this.ensureClimateSettings();
    if (c.regions[id]) { Editor.toast.warning('Регион уже существует'); return; }
    c.regions[id] = JSON.parse(JSON.stringify(c.regions.default || { baseTemp: -5, weights: {} }));
    this.renderClimateTab();
    this.updateJSONPreview();
  };

  Editor.removeClimateRegion = async function (regionId) {
    if (!(await Editor.confirmDialog({ message: `Удалить регион ${regionId}?`, danger: true }))) return;
    const c = this.ensureClimateSettings();
  ...
  Editor.renderSceneClimateSection = function (scene) {
    ...
    const weatherBlock = WEATHER_VARIANT_KEYS.map((wid) => {
      ...
      return `<div class="weather-variant project-info" style="margin:8px 0;">
        <label><input type="checkbox" ${enabled ? 'checked' : ''}
          onchange="Editor.toggleSceneWeatherVariant('${wid}', this.checked)"> ${weatherLabel(wid)}</label>
    ...
    const eventsBlock = events.map((ev, i) => `<div class="weather-event project-info" style="margin:8px 0;">
      <label>Погода: <select onchange="Editor.setWeatherEventField(${i},'weather',this.value)">
        ${WEATHER_VARIANT_KEYS.map(w => `<option value="${w}" ${ev.weather === w ? 'selected' : ''}>${weatherLabel(w)}</option>`).join('')}
      </select></label>
  ...
  Editor.setWeatherEventParams = function (idx, raw) {
    ...
    try {
      ...
      Editor.toast.error('Неверный JSON');
    }
    this.updateJSONPreview();
  };
