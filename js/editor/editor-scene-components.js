  ...
  Object.assign(Editor, {
    ...
    renderComponentEditorCard(comp, index) {
      ...
      switch (type) {
        ...
        case 'repair':
          fields = `${npcSel('npc', p.npc)}
            <div class="form-group"><label>Цена ремонта (зм)</label>
              <input type="number" value="${p.flatCost ?? p.costPerDurability ?? 15}" onchange="Editor.updateComponentParam(${index},'flatCost',parseInt(this.value,10)||15)">
            </div>`;
          break;
        case 'upgrade':
          fields = `${npcSel('npc', p.npc)}
            <div class="form-group"><label>Макс. заточка</label>
              <input type="number" value="${p.maxEnhancement ?? 3}" onchange="Editor.updateComponentParam(${index},'maxEnhancement',parseInt(this.value,10)||3)">
            </div>
            <div class="form-group"><label>Цены по уровням (через запятую)</label>
              <input type="text" value="${(p.costTable || []).join(', ')}" onchange="Editor.updateComponentCostTable(${index},this.value)">
            </div>`;
          break;
        case 'heal':
          fields = `${npcSel('npc', p.npc)}
            <div class="form-group"><label>Стоимость (зм)</label>
              <input type="number" value="${p.cost ?? 25}" onchange="Editor.updateComponentParam(${index},'cost',parseInt(this.value,10)||0)">
            </div>
            <div class="form-group"><label><input type="checkbox" ${p.restoreResources !== false ? 'checked' : ''} onchange="Editor.updateComponentParam(${index},'restoreResources',this.checked)"> Восстановить ресурс</label></div>`;
          break;
        case 'curse_remove':
          fields = `${npcSel('npc', p.npc)}
            <div class="form-group"><label>Базовая стоимость</label>
              <input type="number" value="${p.costBase ?? 50}" onchange="Editor.updateComponentParam(${index},'costBase',parseInt(this.value,10)||50)">
            </div>`;
          break;
        case 'gambling':
          fields = `${npcSel('npc', p.npc)}
            <div class="form-group"><label>Мин. ставка</label>
              <input type="number" value="${p.minBet ?? 5}" onchange="Editor.updateComponentParam(${index},'minBet',parseInt(this.value,10)||5)">
            </div>
            <div class="form-group"><label>Макс. ставка</label>
              <input type="number" value="${p.maxBet ?? 100}" onchange="Editor.updateComponentParam(${index},'maxBet',parseInt(this.value,10)||100)">
            </div>`;
          break;
        case 'crafting':
          fields = `${npcSel('npc', p.npc)}
            <div class="form-group"><label>Рецепты (JSON)</label>
              <textarea rows="4" onchange="Editor.updateComponentRecipes(${index},this.value)">${Editor.escapeHtml(JSON.stringify(p.recipes || [], null, 2))}</textarea>
            </div>`;
          break;
        default:
