...
(function attachEditorNoCodeUx() {
  ...
  Object.assign(Editor, {
    ...
    /** Название сцены → уникальный id (транслит + суффикс _2, _3…). */
    slugifySceneId(name, existing) {
      return this.slugifyId(String(name || '').trim(), '', existing || this.data?.scenes || {});
    },
     ...
     */
    async promptNameAndId(opts) {
      const o = opts || {};
      const name = await Editor.promptDialog({ message: o.namePrompt || 'Название (для игрока):', defaultValue: o.defaultName || '' });
      if (name === null) return null;
      const trimmed = String(name).trim();
      if (!trimmed) {
        Editor.toast.warning('Нужно название');
        return null;
      }
      const existing = o.existing || {};
      let id = this.slugifyId(trimmed, o.prefix || '', existing);
      if (o.allowEditId) {
        const edited = await Editor.promptDialog({ message: 'Технический ID (можно не менять):', defaultValue: id });
        ...
        if (existing[id] || (existing instanceof Set && existing.has(id))) {
          Editor.toast.warning('Такой ID уже есть');
          return null;
        }
    ...
    Editor._renderConditionBuilderBody = function (target, propertyKey, builderId, title) {
      ...
      const presets = `
        <div class="cb-human-presets">
          ...
          <button type="button" class="btn btn-secondary btn-sm" onclick="${this.escapeAttr('Editor.applyHumanConditionPreset(' + JSON.stringify(builderId) + ', ' + JSON.stringify('has_item') + ')')}">Есть предмет</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="${this.escapeAttr('Editor.applyHumanConditionPreset(' + JSON.stringify(builderId) + ', ' + JSON.stringify('quest_active') + ')')}">Квест начат</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="${this.escapeAttr('Editor.applyHumanConditionPreset(' + JSON.stringify(builderId) + ', ' + JSON.stringify('quest_stage') + ')')}">Квест на этапе</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="${this.escapeAttr('Editor.applyHumanConditionPreset(' + JSON.stringify(builderId) + ', ' + JSON.stringify('quest_done') + ')')}">Квест завершён</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="${this.escapeAttr('Editor.applyHumanConditionPreset(' + JSON.stringify(builderId) + ', ' + JSON.stringify('has_gold') + ')')}">Достаточно золота</button>
        </div>`;
  ...
  // updateJSONPreview visibility: initial API / click-recovery own the method.
  // Do not late-wrap here (hooks-only: no Editor.updateJSONPreview = …).

  // applyEditorMode — синхронизировать JSON
  if (Editor.hooks && typeof Editor.hooks.after === 'function') {
    Editor.hooks.after('applyEditorMode', function (result) {
      if (typeof this.applyJsonPreviewVisibility === 'function') this.applyJsonPreviewVisibility();
      return result;
    });
  } else if (typeof console !== 'undefined' && console.warn) {
    console.warn('[editor-no-code-ux] Editor.hooks missing — applyEditorMode extension skipped');
  }
  ...
  // Создание класса — имя-first UX через hooks.replace
  if (Editor.hooks && typeof Editor.hooks.replace === 'function' && typeof Editor.createClass === 'function') {
    Editor.hooks.replace('createClass', async function createClassFriendly() {
      if (typeof this.promptNameAndId === 'function') {
        ...
        if (!r) return;
        const id = r.id;
        const strikeId = typeof this.upsertAbilityToPool === 'function'
          ? this.upsertAbilityToPool({
              id: id + '_strike',
              name: 'Удар',
              cost: 1,
              icon: '⚔️',
    ...
    console.warn('[editor-no-code-ux] Editor.hooks.replace missing — createClass UX skipped');
  }
