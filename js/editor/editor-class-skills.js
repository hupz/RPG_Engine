...
(function attachEditorClassSkills() {
  ...
  // Base implementations resolved lazily (no load-order bind)
  function getBaseRenderClassDetail() {
    // Prefer hooks-owned impl from editor-classes via describe, fallback to current
    return function (id) {
      // Call through unwrapped path is hard; store captured base on first replace
      if (typeof Editor._baseRenderClassDetail === 'function') {
        return Editor._baseRenderClassDetail.call(this, id);
      }
      return '';
    };
  }
  ...
  function renderClassesWithSkillBadges() {
    ...
    const sidebar = ids.map(id => {
      ...
      return `<button type="button" class="class-pick ${active}" onclick="${this.escapeAttr('Editor.selectClassToEdit(' + JSON.stringify(id) + ')')}">
  ...
  if (Editor.hooks && typeof Editor.hooks.replace === 'function') {
    Editor.hooks.replace('renderClasses', renderClassesWithSkillBadges, 'editor-class-skills');
  } else {
    Editor.renderClasses = renderClassesWithSkillBadges;
  }
