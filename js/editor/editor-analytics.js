...
(function attachEditorAnalytics() {
  ...
  Object.assign(Editor, {
    ...
    async clearAnalyticsData() {
      ...
      if (!pk) return;
      if (!(await Editor.confirmDialog({ message: 'Удалить всю локальную аналитику для этого проекта? Действие необратимо.', danger: true }))) return;
      try {
  ...
  if (Editor.hooks?.after) {
    ...
    console.warn('[editor-analytics] Editor.hooks missing — extension skipped');
  }
