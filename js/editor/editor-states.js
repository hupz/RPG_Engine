...
(function attachEditorStates() {
  ...
  Object.assign(Editor, {
    ...
    async removeSceneState(idx) {
      const scene = this.data.scenes[this.currentScene];
      if (!scene?.states) return;
      if (!(await Editor.confirmDialog({ message: 'Удалить состояние #' + (idx + 1) + '?', danger: true }))) return;
  ...
  function appendSceneStatesPanel() {
    ...
    if (container.querySelector('.scene-states-panel, #states-list')) return;
    const baseText = container.querySelector('#scene-text');
    if (!baseText || !baseText.parentElement) return;
    if (typeof this.renderSceneStatesPanel !== 'function') return;
  ...
  if (Editor.hooks && typeof Editor.hooks.after === 'function') {
    Editor.hooks.after('renderSceneEditor', function () {
      appendSceneStatesPanel.call(this);
    });
  }
})();
