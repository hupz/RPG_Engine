...
(function attachEditorRecipes() {
  ...
  Object.assign(Editor, {
    ...
    async createRecipe() {
      if (typeof this.ensureCraftingData === 'function') this.ensureCraftingData();
      const name = await Editor.promptDialog({ message: 'Название рецепта:', defaultValue: 'Новый рецепт' });
      if (!name?.trim()) return;
    ...
    saveRecipe(id) {
      ...
      if (errors.length) {
        Editor.toast.error('Ошибки:\n' + errors.join('\n'));
        return;
      }
      recipe.name = recipe.name.trim();
      this.updateJSONPreview();
      this.recipeWizardStep = 0;
      Editor.toast.success('✅ Рецепт сохранён');
      this.renderRecipes();
    },

    async deleteRecipe(id) {
      if (!this.data?.recipes?.[id]) return;
      if (!(await Editor.confirmDialog({ message: `Удалить рецепт «${this.data.recipes[id].name || id}»?`, danger: true }))) return;
    ...
    addRecipeIngredient(id) {
      ...
      if (recipe.ingredients.length >= MAX_RECIPE_INGREDIENTS) {
        Editor.toast.warning(`Максимум ${MAX_RECIPE_INGREDIENTS} ингредиента.`);
        return;
      }
    ...
    importRecipesJson() {
      ...
      input.onchange = async (e) => {
        ...
        try {
          ...
          this.renderRecipes();
          this.updateJSONPreview();
          Editor.toast.success('✅ Рецепты импортированы');
        } catch (err) {
          Editor.toast.error('❌ ' + err.message);
        }
      };
      input.click();
    },
    ...
    goToItemsTabForRecipe(id) {
      ...
      Editor.toast.info('Создайте предмет с ID, который выберете в рецепте «' + (recipe.name || id) + '».');
    },
    ...
    testRecipe(id) {
      ...
      const errors = this.validateRecipe(recipe);
      if (errors.length) {
        Editor.toast.error('Нельзя протестировать:\n' + errors.join('\n'));
        return;
      }
      const logs = [];
      const inv = {};
    ...
    renderRecipeResultSelect(id, recipe) {
      ...
      const preview = cur && items[cur]
        ...
        : `<button type="button" class="btn btn-secondary" onclick="${this.escapeAttr('Editor.goToItemsTabForRecipe(' + JSON.stringify(id) + ')')}">Создать предмет во вкладке Items</button>`;
    ...
    renderRecipeIngredientsEditor(id, recipe) {
      ...
      const rows = (recipe.ingredients || []).map((row, idx) => {
        ...
        return `<div class="craft-ing-row">
          <div class="craft-ing-head">#${idx + 1}
            <button type="button" class="btn btn-danger btn-sm" onclick="${this.escapeAttr('Editor.removeRecipeIngredient(' + JSON.stringify(rid) + ', ' + idx + ')')}">×</button></div>
      ...
      return `<div class="project-info">
        <div class="craft-ing-toolbar"><h4>Ингредиенты (${MIN_RECIPE_INGREDIENTS}–${MAX_RECIPE_INGREDIENTS})</h4>
          ${canAdd ? `<button type="button" class="btn btn-secondary" onclick="${this.escapeAttr('Editor.addRecipeIngredient(' + JSON.stringify(rid) + ')')}">+ Добавить</button>` : ''}</div>
    ...
    renderRecipeWizard(id) {
      ...
      const steps = [
        ...
        `<div class="craft-wizard-step"><h4>Шаг 5: Проверка</h4>
          ...
          <button type="button" class="btn btn-primary" onclick="${this.escapeAttr('Editor.saveRecipe(' + JSON.stringify(rid) + ')')}">💾 Сохранить рецепт</button></div>`
      ];
      ...
      return `<div class="quest-detail-card craft-detail-card">
        <div class="quest-detail-head">
          ...
          <div class="craft-detail-actions">
            <button type="button" class="btn btn-primary" onclick="${this.escapeAttr('Editor.saveRecipe(' + JSON.stringify(rid) + ')')}">💾 Сохранить</button>
            <button type="button" class="btn btn-secondary" onclick="${this.escapeAttr('Editor.testRecipe(' + JSON.stringify(rid) + ')')}">🧪 Тест</button>
            <button type="button" class="btn btn-danger" onclick="${this.escapeAttr('Editor.deleteRecipe(' + JSON.stringify(rid) + ')')}">🗑️ Удалить</button>
          </div>
        </div>
    ...
    renderRecipes() {
      ...
      const sidebar = ids.map((rid) => {
        ...
        return `<button type="button" class="quest-pick${active}${afford}" onclick="${this.escapeAttr('Editor.selectRecipeToEdit(' + JSON.stringify(rid) + ')')}">${r.icon || '🔨'} ${this.escapeHtml(r.name || rid)}</button>`;
      }).join('');
