...
(function attachEditorBeasts() {
  ...
  Object.assign(Editor, {
    ...
    bindBeastsEditorEvents(container) {
      if (!container || container.dataset.beastsBound === '1') return;
      container.dataset.beastsBound = '1';
      container.addEventListener('click', (e) => {
        const delBtn = e.target.closest('[data-beast-delete]');
        if (delBtn) {
          e.preventDefault();
          this.deleteBeast(delBtn.getAttribute('data-beast-delete'));
          return;
        }
        const editBtn = e.target.closest('[data-beast-edit]');
        if (editBtn) {
          e.preventDefault();
          this.editBeast(editBtn.getAttribute('data-beast-edit'));
        }
      });
    },
      ...
      const cards = ids.map((id) => {
        ...
        return `<div class="beast-card">
          ...
          <div class="beast-card-actions">
            <button type="button" class="btn btn-secondary" data-beast-edit="${safeId}">Редактировать</button>
            <button type="button" class="btn btn-danger" data-beast-delete="${safeId}">Удалить</button>
          </div>
        </div>`;
      }).join('');
    ...
    async createBeast() {
      const id = await Editor.promptDialog({ message: 'ID зверя (латиница, без пробелов):', defaultValue: 'new_beast' });
      if (!id || !/^[a-z][a-z0-9_]*$/i.test(id)) {
        Editor.toast.warning('ID: латиница, без пробелов');
        return;
      }
      this.ensureBeasts();
      if (this.data.beasts[id]) {
        Editor.toast.warning('Зверь с таким ID уже есть');
        return;
      }
      this.data.beasts[id] = {
        id,
    ...
    async deleteBeast(id) {
      if (!id || !this.data?.beasts?.[id]) {
        Editor.toast.warning('Зверь не найден');
        return;
      }
      const name = this.data.beasts[id].name || id;
      if (!(await Editor.confirmDialog({ message: `Удалить зверя «${name}» (${id})?`, danger: true }))) return;
      if (typeof BeastSystem !== 'undefined' && BeastSystem.removeBeast) {
        BeastSystem.removeBeast(this.data, id);
      } else {
    ...
    renderBeastEditForm(id) {
      ...
      return `<div class="beast-edit-form">
        ...
        <button type="button" class="btn btn-secondary" onclick="${this.escapeAttr('Editor.addCustomBeastAbility(' + JSON.stringify(id) + ')')}">+ Добавить свою</button>
        ...
        <div class="form-actions" style="margin-top:16px;">
          ...
          <button type="button" class="btn btn-danger" data-beast-delete="${this.escapeAttr(id)}">🗑 Удалить</button>
        </div>
      </div>`;
    },
    ...
    async addCustomBeastAbility(id) {
      const key = await Editor.promptDialog({ message: 'ID способности (латиница):', defaultValue: 'custom_trait' });
      if (!key) return;
    ...
    saveBeastEdit() {
      ...
      this.renderBeasts();
      this.updateJSONPreview();
      Editor.toast.success('Зверь сохранён в данных (не забудьте экспорт JSON).');
    },
  ...
  if (Editor.hooks?.after) {
    ...
    console.warn('[editor-beasts] Editor.hooks missing — extension skipped');
  }
})();
