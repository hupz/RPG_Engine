...
(function attachEditorRaces() {
  ...
  Object.assign(Editor, {
    ...
    async createRace() {
      ...
      const id = await Editor.promptDialog({ message: 'ID расы (латиница, snake_case):', defaultValue: sys === 'pf2e' ? 'new_ancestry_pf2e' : 'new_race' });
      if (!id || !/^[a-z][a-z0-9_]*$/i.test(id)) {
        Editor.toast.warning('ID должен начинаться с буквы и содержать только латиницу, цифры и _');
        return;
      }
      if (this.data.races[id]) {
        Editor.toast.warning('Раса с таким ID уже есть');
        return;
      }
      if (sys === 'pf2e') {
        this.data.races[id] = {
    ...
    async deleteRace(id) {
      if (!id || !(await Editor.confirmDialog({ message: 'Удалить расу «' + id + '»?', danger: true }))) return;
  ...
  if (Editor.hooks && typeof Editor.hooks.after === 'function') {
    Editor.hooks.after('renderAll', function (result) {
      if (typeof this.renderRaces === 'function') this.renderRaces();
      return result;
    });
  } else if (typeof console !== 'undefined' && console.warn) {
    console.warn('[editor-races] Editor.hooks missing — extension skipped');
  }
})();
