...
(function attachEditorSidebarPanels() {
  ...
  Object.assign(Editor, {
    ...
    ensureEnemies() {
      ...
      if (!this.data.creatureTypes || typeof this.data.creatureTypes !== 'object') {
        this.data.creatureTypes = {
          beast: 'Звери',
          humanoid: 'Гуманоиды',
          giant: 'Великаны',
      ...
      Object.values(this.data.enemies).forEach((e) => {
        if (e && !e.creatureType) e.creatureType = 'humanoid';
      });
    },
    ...
    renderEnemyCreatureTypeSelect(id, e) {
      const eid = this.escapeAttr(id);
      const types = this.data?.creatureTypes || {
        beast: 'Звери',
        humanoid: 'Гуманоиды',
        giant: 'Великаны',
        elemental: 'Элементали'
      };
      const cur = e.creatureType || 'humanoid';
      let opts = '';
      Object.entries(types).forEach(([tid, label]) => {
        const name = typeof label === 'string' ? label : (label?.name || tid);
        opts += `<option value="${this.escapeAttr(tid)}" ${cur === tid ? 'selected' : ''}>${this.escapeHtml(name)}</option>`;
      });
      return `<div class="form-group"><label>Тип существа</label>
        <select onchange="Editor.updateEnemy('${eid}','creatureType',this.value)">${opts}</select></div>`;
    },
    ...
    renderEnemyDetail(id) {
      ...
      return `<div class="quest-detail-card">
        <div class="quest-detail-head">
          ...
          <button type="button" class="btn btn-danger" onclick="${this.escapeAttr('Editor.deleteEnemy(' + JSON.stringify(eid) + ')')}">🗑 Удалить</button>
        </div>
        ...
        ${this.renderEnemyCreatureTypeSelect(id, e)}
        <div class="form-group">
          <label><input type="checkbox" ${e.boss ? 'checked' : ''}
            onchange="Editor.updateEnemy('${eid}','boss',this.checked)"> Босс (доп. множитель HP из вкладки «Баланс»)</label>
        </div>
        <div class="form-group">
          <label><input type="checkbox" ${e.scaleWithPlayerLevel !== false ? 'checked' : ''}
            onchange="Editor.updateEnemyScaleWithLevel('${eid}', this.checked)"> Масштабировать с уровнем игрока</label>
          <p class="hint">Если снято — всегда базовые статы из JSON (для особых NPC/боссов).</p>
        </div>
        ...
        ${typeof this.renderEnemyBehaviorSection === 'function' ? this.renderEnemyBehaviorSection(id) : ''}
        ${typeof this.renderEnemyFactionKillSection === 'function' ? this.renderEnemyFactionKillSection(id) : ''}
        ${typeof this.renderEnemySoundSection === 'function' ? this.renderEnemySoundSection(id) : ''}
      </div>`;
    },
    ...
    renderEnemies() {
      ...
      c.innerHTML = this.renderEditorListLayout({
        ...
        getLabel: (id) => {
          const e = this.data.enemies[id];
          const boss = e?.boss ? ' ☠' : '';
          const typeRaw = e?.creatureType;
          const typeName = typeRaw
            ? (typeof this.data.creatureTypes?.[typeRaw] === 'string'
              ? this.data.creatureTypes[typeRaw]
              : this.data.creatureTypes?.[typeRaw]?.name || typeRaw)
            : '';
          const typeTag = typeName ? ` · ${typeName}` : '';
          return (e?.name || id) + typeTag + boss;
        },
    ...
    renderNpcDetail(id) {
      ...
      return `<div class="quest-detail-card npc-editor">
        <div class="quest-detail-head">
          ...
          <button type="button" class="btn btn-danger" onclick="${this.escapeAttr('Editor.deleteNPC(' + JSON.stringify(nid) + ')')}">🗑 Удалить</button>
        </div>
        <div class="form-group"><label>ID</label><input value="${this.escapeHtml(id)}" disabled></div>
        <div class="form-group"><label>Имя</label>
        ...
        ${typeof this.renderNpcReputationSection === 'function' ? this.renderNpcReputationSection(id) : ''}
      </div>`;
    },
    ...
    renderAbilities() {
      ...
      c.innerHTML = this.renderEditorListLayout({
        ...
        detailHtml: detail,
        emptyHint: 'Единый пул умений. Назначайте их классам во вкладке «Классы».'
      });
    }
  });
  ...
  Editor.updateEnemy = function (id, f, val) {
    if (f === 'soundAttack' || f === 'soundHit') {
      origUpdateEnemy.call(this, id, f, val);
      if (!val) {
        delete this.data.enemies[id]?.[f];
      }
      this.updateJSONPreview();
      return;
    }
    origUpdateEnemy.call(this, id, f, val);
    if (f === 'name' || f === 'boss' || f === 'creatureType' || f === 'faction'
      || f === 'factionImportant' || f === 'reputationOnKill') this.renderEnemies();
  };

  Editor.createEnemy = async function () {
    const id = await Editor.promptDialog({ message: 'ID врага:' });
    if (!id) return;
    this.ensureEnemies();
    if (this.data.enemies[id]) {
      Editor.toast.warning('Враг с таким ID уже есть');
      return;
    }
    this.data.enemies[id] = {
      id, name: 'Новый враг', creatureType: 'humanoid', hp: 10, maxHp: 10, ac: 12,
  ...
  Editor.deleteEnemy = async function (id) {
    if (!(await Editor.confirmDialog({ message: 'Удалить врага?', danger: true }))) return;
  ...
  Editor.createNPC = async function () {
    const id = await Editor.promptDialog({ message: 'ID NPC:' });
    if (!id) return;
    this.ensureNpcs();
    if (this.data.npcs[id]) return;
    this.data.npcs[id] = {
  ...
  Editor.deleteNPC = async function (id) {
    if (!(await Editor.confirmDialog({ message: 'Удалить NPC?', danger: true }))) return;
  ...
  Editor.deleteGlobalAbility = async function (id) {
    if (!(await Editor.confirmDialog({ message: 'Удалить умение из пула? Оно также будет убрано у всех классов.', danger: true }))) return;
    ...
    Object.values(this.data.classes || {}).forEach((cls) => {
      ...
      cls.abilities = cls.abilities
        .map((ab) => (typeof ab === 'string' ? ab : ab?.id))
        .filter((aid) => aid && aid !== id);
    });
    const ids = this.getGlobalAbilityIds();
    this.editingGlobalAbilityId = ids[0] || null;
    this.renderAbilities();
    if (typeof this.renderClasses === 'function' && this.editingClassId) this.renderClasses();
    this.updateJSONPreview();
  };
})();
