...
(function attachEditorWizards() {
  ...
  Object.assign(Editor, {
    ...
    renderSceneWizardsPanel() {
      if (!this._wizardOpen) {
        return `<div class="scene-wizards-bar">
          ...
          <button type="button" class="btn btn-secondary btn-sm" onclick="Editor.openScenarioWizard('shop')">🛒 Магазин</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="Editor.openScenarioWizard('combat')">⚔️ Бой</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="Editor.openScenarioWizard('item')">🎁 Выдать предмет</button>
          <button type="button" class="btn btn-secondary btn-sm" onclick="Editor.openScenarioWizard('quest')">📜 Старт квеста</button>
        </div>`;
      }
    ...
    applyWizardShop() {
      ...
      this.renderSceneEditor();
      Editor.toast.success('Магазин добавлен на сцену.');
    },
    ...
    applyWizardCombat() {
      ...
      if (!ids.length) {
        Editor.toast.warning('Выберите хотя бы одного врага');
        return;
      }
      scene.combat = ids;
      ...
      this.updateJSONPreview();
      this.renderSceneEditor();
      Editor.toast.success('Бой настроен.');
    },
    ...
    applyWizardItem() {
      ...
      Editor.toast.info('Предмет «' + name + '» будет выдан при входе.');
    },
    ...
    applyWizardQuest() {
      ...
      const qid = document.getElementById('wiz-quest-id')?.value;
      if (!qid) {
        Editor.toast.warning('Выберите квест');
        return;
      }
      ...
      Editor.toast.success('Выбор добавлен: квест «' + title + '».');
    },
  ...
  // renderChoiceEditor returns HTML — replace via hooks.
  // NEVER call public Editor.renderChoiceEditor from inside the replacement
  // (that is the hooks wrapper → infinite recursion / "too much recursion").
  if (Editor.hooks && typeof Editor.hooks.replace === 'function') {
    function injectChoiceQuestFields(html, c, i) {
      ...
      if (html.includes('</div>')) {
        ...
        return html.slice(0, idx) + questBlock + html.slice(idx);
      }
      return html + questBlock;
    }

    // Capture prior _impl from replace()'s return value — not Editor.renderChoiceEditor
    const baseChoice = Editor.hooks.replace('renderChoiceEditor', function (c, i, allScenes) {
      const html = typeof baseChoice === 'function'
        ? baseChoice.call(this, c, i, allScenes)
        : '';
      return injectChoiceQuestFields.call(this, html, c, i);
    }, 'editor-wizards');
  }
