...
(function attachEditorWorldMap() {
  ...
  Object.assign(Editor, {
    ...
    async createMapFromCurrentScene() {
      if (!this.currentScene) return;
      const scene = this.data.scenes[this.currentScene];
      const suggested = (this.currentScene || 'place').replace(/[^a-z0-9_]+/gi, '_').replace(/^_|_$/g, '').toLowerCase() || 'new_place';
      let id;
      const labelPrompt = await Editor.promptDialog({ message: 'Название точки на карте:', defaultValue: scene.location || suggested });
      if (labelPrompt === null) return;
      if (typeof this.slugifyId === 'function') {
        id = this.slugifyId(labelPrompt || suggested, '', this.data.worldMap || {});
      } else {
        id = await Editor.promptDialog({ message: 'ID точки на карте (латиница):', defaultValue: suggested });
        if (!id || !/^[a-z][a-z0-9_]*$/i.test(id)) {
          Editor.toast.warning('ID: только латиница, цифры и _');
          return;
        }
      }
      this.ensureWorldMap();
      if (this.data.worldMap[id] && !(await Editor.confirmDialog({ message: 'Точка "' + id + '" уже есть. Привязать сцену к ней?' }))) return;
      ...
      this.renderWorldMap();
      this.updateJSONPreview();
      Editor.toast.success('✅ Точка «' + id + '» создана и привязана к сцене.');
    },
    ...
    renderMapLocationDetail(id) {
      ...
      return `<${D} class="quest-detail-card worldmap-card">
        <${D} class="quest-detail-head">
          ...
          <button type="button" class="btn btn-danger" onclick="${this.escapeAttr('Editor.deleteMapLocation(' + JSON.stringify(mid) + ')')}">🗑 Удалить</button>
        </${D}>
        ...
        <${D} class="form-group"><label>Условие показа на карте</label>
          ...
          <${D} style="display:flex;flex-wrap:wrap;gap:8px;align-items:center;margin-bottom:10px;">
            ...
            <button type="button" class="btn btn-secondary" style="font-size:11px;" onclick="${this.escapeAttr('Editor.clearMapQuestShowIf(' + JSON.stringify(mid) + ')')}">Сбросить квест</button>
          </${D}>
          <${D} class="grid-2">
    ...
    async createMapLocation() {
      const id = await Editor.promptDialog({ message: 'ID точки (латиница, например forest_camp):', defaultValue: 'new_place' });
      if (!id || !/^[a-z][a-z0-9_]*$/i.test(id)) {
        Editor.toast.warning('ID: только латиница, цифры и _');
        return;
      }
      this.ensureWorldMap();
      if (this.data.worldMap[id]) {
        Editor.toast.warning('Точка уже существует');
        return;
      }
    ...
    async deleteMapLocation(id) {
      if (!(await Editor.confirmDialog({ message: 'Удалить точку «' + id + '» с карты? mapLocation у сцен не сбросится автоматически.', danger: true }))) return;
  ...
  function decorateSceneListMapBadges() {
  ...
  // hooks-only: never Editor.renderSceneList = ...
  if (Editor.hooks && typeof Editor.hooks.after === 'function') {
    Editor.hooks.after('renderSceneList', function (result) {
      decorateSceneListMapBadges.call(this);
      return result;
    });
  } else if (typeof console !== 'undefined' && console.warn) {
    console.warn('[editor-worldmap] Editor.hooks missing — map badges not attached to renderSceneList');
  }

  function appendMapLocationField() {
    ...
    if (typeof this.renderMapLocationField !== 'function') return;
  ...
  if (Editor.hooks && typeof Editor.hooks.after === 'function') {
    Editor.hooks.after('renderSceneEditor', function () {
      appendMapLocationField.call(this);
    });
  }
