...
(function attachEditorWorldHierarchy() {
  ...
  Object.assign(Editor, {
    ...
    renderHierarchyTreeHtml(tree) {
      ...
      tree.forEach((world) => {
        html += `<li><button type="button" class="wh-tree-btn" onclick="${this.escapeAttr('Editor.selectHierarchyNode(' + JSON.stringify('world') + ', ' + JSON.stringify(world.id) + ')')}">${world.icon} ${this.escapeHtml(world.name)}</button>`;
        html += '<ul>';
        world.regions.forEach((region) => {
          html += `<li><button type="button" class="wh-tree-btn wh-tree-btn--region" onclick="${this.escapeAttr('Editor.selectHierarchyNode(' + JSON.stringify('region') + ', ' + JSON.stringify(region.id) + ')')}">${region.icon} ${this.escapeHtml(region.name)}</button>`;
          html += '<ul>';
          region.hubs.forEach((hub) => {
            html += `<li><button type="button" class="wh-tree-btn wh-tree-btn--hub" onclick="${this.escapeAttr('Editor.selectHierarchyNode(' + JSON.stringify('hub') + ', ' + JSON.stringify(hub.id) + ')')}">${hub.icon} ${this.escapeHtml(hub.name)}</button>`;
            html += '<ul>';
            hub.scenes.forEach((sc) => {
              const icon = sc.type === 'shop' ? '🏪' : sc.type === 'tavern' ? '🍺' : '📍';
              html += `<li><button type="button" class="wh-tree-btn wh-tree-btn--scene" onclick="${this.escapeAttr('Editor.selectHierarchyNode(' + JSON.stringify('scene') + ', ' + JSON.stringify(sc.id) + ')')}">${icon} ${this.escapeHtml(sc.name)}</button></li>`;
            });
            html += '</ul></li>';
          });
          html += '</ul></li>';
    ...
    renderHubEditPanel(hubId) {
      ...
      const sceneList = hub.scenes.map((sid) => {
        ...
        return `<li>${this.escapeHtml(sc?.location || sid)} <code>${sid}</code>
          <button type="button" class="btn btn-secondary" style="font-size:11px;" onclick="${this.escapeAttr('Editor.selectScene(' + JSON.stringify(sid) + ')')}">Редактор сцены</button>
          <button type="button" class="btn btn-danger" style="font-size:11px;" onclick="${this.escapeAttr('Editor.detachSceneFromHub(' + JSON.stringify(sid) + ')')}">Отвязать</button></li>`;
      ...
      return `<h3>${WH.getHubIcon(hub)} Редактирование хаба</h3>
        ...
        <${D} style="display:flex;gap:8px;flex-wrap:wrap;margin-top:10px;">
          <button type="button" class="btn btn-primary" onclick="Editor.openSceneWizard()">+ Новая сцена</button>
          <select id="wh-attach-scene-select" class="form-control" style="max-width:220px;">${sceneOpts}</select>
          <button type="button" class="btn btn-secondary" onclick="${this.escapeAttr('Editor.attachSceneToHub(' + JSON.stringify(hubId) + ', document.getElementById(\'wh-attach-scene-select\').value)')}">Привязать сцену</button>
        </${D}>`;
    },
    ...
    renderHierarchyScenePanel(sceneId) {
      ...
      return `<h3>📍 ${this.escapeHtml(scene.location || sceneId)}</h3>
        ...
        <${D} style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;">
          <button type="button" class="btn btn-primary" onclick="${this.escapeAttr('Editor.selectScene(' + JSON.stringify(sceneId) + ')')}">🎬 Открыть в редакторе сцен</button>
          ${hubId ? `<button type="button" class="btn btn-danger" onclick="${this.escapeAttr('Editor.detachSceneFromHub(' + JSON.stringify(sceneId) + ')')}">🚫 Отвязать от хаба</button>` : `<button type="button" class="btn btn-secondary" onclick="${this.escapeAttr('Editor.promptAttachSceneToHub(' + JSON.stringify(sceneId) + ')')}">Привязать к хабу</button>`}
        </${D}>`;
    },
    ...
    setSceneOwnStateJson(sceneId, raw) {
      try {
        ...
        Editor.toast.error('Неверный JSON: ' + e.message);
      }
    },
    ...
    async promptAttachSceneToHub(sceneId) {
      const hubIds = Object.keys(this.data.hubs || {});
      if (!hubIds.length) {
        Editor.toast.warning('Сначала создайте хаб');
        return;
      }
      const hubId = await Editor.promptDialog({ message: 'ID хаба:\n' + hubIds.join(', '), defaultValue: hubIds[0] });
      if (hubId && this.data.hubs[hubId]) this.attachSceneToHub(hubId, sceneId);
    },

    async createWorld() {
      const id = await Editor.promptDialog({ message: 'ID мира (латиница):', defaultValue: 'world_main' });
      if (!id || !/^[a-z][a-z0-9_]*$/i.test(id)) return;
      if (this.data.worlds[id]) {
        Editor.toast.warning('Уже существует');
        return;
      }
    ...
    async createRegion() {
      ...
      const id = await Editor.promptDialog({ message: 'ID региона:', defaultValue: 'region_new' });
      if (!id || !/^[a-z][a-z0-9_]*$/i.test(id)) return;
    ...
    async createHub() {
      ...
      if (!regionIds.length) {
        Editor.toast.warning('Сначала создайте регион');
        return;
      }
      const parent = regionIds.includes('region_tihaya_river') ? 'region_tihaya_river' : regionIds[0];
      const id = await Editor.promptDialog({ message: 'ID хаба:', defaultValue: 'hub_new' });
      if (!id || !/^[a-z][a-z0-9_]*$/i.test(id)) return;
      this.data.hubs[id] = {
        name: 'Новый хаб',
        parent,
  ...
  function injectHierarchyMapDiagram() {
    const c = document.getElementById('worldmap-editor');
    if (!c || !Editor.data) return;
    const diagram = Editor.renderHierarchyMapDiagram?.();
    const detail = c.querySelector('.quest-manager-detail');
    if (detail && diagram) {
      const wrap = document.createElement(typeof D !== 'undefined' ? D : 'div');
      wrap.innerHTML = diagram;
      if (wrap.firstElementChild) detail.insertBefore(wrap.firstElementChild, detail.firstChild);
    }
  }
  function injectSceneHubBadges() {
    ...
    document.querySelectorAll('.scene-item').forEach((el) => {
      ...
      const sid = m?.[1];
      if (!sid) return;
      const hubId = WH.getHubIdForScene(Editor.data, sid);
      if (hubId) {
  ...
  if (Editor.hooks && typeof Editor.hooks.after === 'function') {
    Editor.hooks.after('renderWorldMap', function () {
      injectHierarchyMapDiagram();
    });
    Editor.hooks.after('switchTab', function (result, args) {
      if (args && args[0] === 'world') this.renderWorldHierarchy?.();
      return result;
    });
    ...
    Editor.hooks.after('renderSceneList', function () {
      injectSceneHubBadges();
    });
  } else if (typeof console !== 'undefined' && console.warn) {
    console.warn('[editor-world-hierarchy] Editor.hooks missing — extension skipped');
  }
