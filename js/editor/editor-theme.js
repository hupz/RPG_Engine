...
(function attachEditorTheme() {
  ...
  Object.assign(Editor, {
    /** Применить тему из data.theme к превью и (опционально) к корню редактора */
    applyThemeFromData() {
      if (!this.data?.theme) {
      ...
      if (preview) {
        ThemeSystem.apply(this.data.theme, preview);
      }
    },
    ...
    saveThemeAsUserPreset() {
      ...
      Editor.toast.success('Пресет «' + saved.label + '» сохранён в «Мои».');
    },

    async deleteUserThemePreset(presetId) {
      if (!presetId || !String(presetId).startsWith('user_')) return;
      if (!(await Editor.confirmDialog({ message: 'Удалить пользовательский пресет?', danger: true }))) return;
