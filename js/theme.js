...
const ThemeSystem = {
  ...
  MODE_STORAGE_KEY: 'rpgengine_theme_mode',
  LIGHT_PRESET: 'parchment',
  DARK_PRESET: 'dark',
  _toggleButtons: [],
  ...
  get DEFAULT() {
    return this.buildThemeFromPreset(this.DARK_PRESET);
  },
  ...
  expandCore(core) {
    ...
    const panel = core.panel ? this.normalizeHex(core.panel) : null;
    const pageBg = core.pageBg ? this.normalizeHex(core.pageBg) : null;
    const textMuted = core.textMuted ? this.normalizeHex(core.textMuted) : null;
    const infoBase = core.info ? this.normalizeHex(core.info) : accent;
    const onAccent = this.contrastText(accent, '#FFFFFF', '#121212');
    const onSuccess = this.contrastText(success, '#FFFFFF', '#121212');

    return {
      pageBg: pageBg || this.darken(bg, isDark ? 0.08 : 0.06),
      paper: bg,
      paperDark: panel ? this.darken(panel, 0.08) : (isDark ? this.darken(bg, 0.12) : this.darken(bg, 0.05)),
      paperShadow: border,
      cardBg: panel || (isDark ? this.lighten(bg, 0.08) : '#FFFFFF'),
      ink: text,
      inkLight: textMuted || (isDark ? '#B0B0B0' : this.mixColors(text, bg, 0.55)),
      inkFaint: isDark ? '#9AA0AA' : this.mixColors(text, bg, 0.72),
      accent,
      ...
      info: infoBase,
      infoLight: this.lighten(infoBase, 0.18),
      border,
      borderDark: isDark ? this.lighten(border, 0.08) : this.darken(border, 0.12),
  ...
  buildThemeFromPreset(presetId) {
    const def = this.getPresetDefinition(presetId) || this.BUILTIN_PRESETS.dark;
    ...
    if (def.core) {
      ...
      core = { ...this.BUILTIN_PRESETS.dark.core };
      colors = this.expandCore(core);
    }
  ...
  resolve(theme) {
    ...
    const base = theme.core
      ...
      : JSON.parse(JSON.stringify(this.buildThemeFromPreset(this.DARK_PRESET)));
  ...
  getStoredMode() {
    try {
      const v = localStorage.getItem(this.MODE_STORAGE_KEY);
  ...
  setStoredMode(mode) {
    try {
      localStorage.setItem(this.MODE_STORAGE_KEY, mode === 'light' ? 'light' : 'dark');
    } catch (_) { /* ignore */ }
  },

  getModePresetId(mode) {
    return mode === 'light' ? this.LIGHT_PRESET : this.DARK_PRESET;
  },

  /** Шрифты одинаковы в светлой и тёмной теме — меняются только цвета */
  getModeFonts() {
    const light = this.BUILTIN_PRESETS[this.LIGHT_PRESET];
    return { ...(light?.fonts || this.DEFAULT_FONTS) };
  },

  applyChromeTokens(resolved, root = document.documentElement) {
    const isDark = this.getStoredMode() === 'dark';

    if (isDark) {
      root.style.setProperty('--editor-header-bg', '#484850');
      root.style.setProperty('--editor-header-fg', '#ECECEF');
      root.style.setProperty('--editor-surface-info', 'color-mix(in srgb, var(--info) 16%, var(--card-bg))');
      root.style.setProperty('--editor-surface-warn', 'color-mix(in srgb, var(--gold) 14%, var(--card-bg))');
      root.style.setProperty('--editor-surface-success', 'color-mix(in srgb, var(--success) 14%, var(--card-bg))');
      root.style.setProperty('--editor-surface-muted', 'var(--paper-dark)');
      root.style.setProperty('--input-bg', 'var(--paper-dark)');
      root.style.setProperty('--input-fg', 'var(--ink)');
    } else {
      root.style.setProperty('--editor-header-bg', '#F4F5F7');
      root.style.setProperty('--editor-header-fg', '#1A1A22');
      root.style.setProperty('--editor-surface-info', '#E8F0FE');
      root.style.setProperty('--editor-surface-warn', '#FFF8E1');
      root.style.setProperty('--editor-surface-success', '#E8F5E9');
      root.style.setProperty('--editor-surface-muted', '#ECEEF2');
      root.style.setProperty('--input-bg', '#FFFFFF');
      root.style.setProperty('--input-fg', 'var(--ink)');
    }
  },

  applyMode(mode, root = document.documentElement) {
    const normalized = mode === 'light' ? 'light' : 'dark';
    this.setStoredMode(normalized);
    root.dataset.themeMode = normalized;
    if (document.body) document.body.dataset.themeMode = normalized;
    const theme = this.buildThemeFromPreset(this.getModePresetId(normalized));
    theme.fonts = this.getModeFonts();
    const resolved = this.apply(theme, root);
    this.applyChromeTokens(resolved, root);
    this.refreshToggleButtons();
    try {
      document.dispatchEvent(new CustomEvent('editor-theme-mode', { detail: { mode: normalized } }));
    } catch (_) { /* ignore */ }
    return resolved;
  },

  applyStoredMode(root = document.documentElement) {
    return this.applyMode(this.getStoredMode(), root);
  },

  toggleMode() {
    const next = this.getStoredMode() === 'dark' ? 'light' : 'dark';
    return this.applyMode(next);
  },

  refreshToggleButtons() {
    this._toggleButtons.forEach((btn) => this.updateToggleButton(btn));
  },
  ...
  registerToggleButton(btn) {
    if (!btn || this._toggleButtons.includes(btn)) return;
  ...
  initAppTheme(root = document.documentElement) {
    return this.applyStoredMode(root);
  },
  ...
  syncMetaTheme(data) {
    ...
    if (t?.preset) {
      ...
      data.meta.theme = this.DARK_PRESET;
    }
  },
  ...
  applyForApp(projectTheme, root = document.documentElement) {
    const modeTheme = this.buildThemeFromPreset(this.getModePresetId(this.getStoredMode()));
    if (!projectTheme || typeof projectTheme !== 'object') {
      return this.applyMode(this.getStoredMode(), root);
    }
    if (projectTheme.preset === this.LIGHT_PRESET || projectTheme.preset === this.DARK_PRESET) {
      return this.applyMode(this.getStoredMode(), root);
    }
    if (projectTheme.core || projectTheme.colors) {
      const merged = this.resolve(projectTheme);
      if (this.getStoredMode() === 'dark') {
        merged.colors = {
          ...modeTheme.colors,
          ...merged.colors,
          pageBg: modeTheme.colors.pageBg,
          paper: modeTheme.colors.paper,
          cardBg: modeTheme.colors.cardBg,
          ink: modeTheme.colors.ink,
          inkLight: modeTheme.colors.inkLight,
          inkFaint: modeTheme.colors.inkFaint,
          border: modeTheme.colors.border,
          borderDark: modeTheme.colors.borderDark
        };
      }
      root.dataset.themeMode = this.getStoredMode();
      const applied = this.apply(merged, root);
      this.refreshToggleButtons();
      return applied;
    }
    return this.applyMode(this.getStoredMode(), root);
  ...
  ensureInData(data) {
    ...
    const metaTheme = data.meta.theme || this.DARK_PRESET;
