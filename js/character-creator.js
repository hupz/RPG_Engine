  ...
  const CharacterCreator = {
    ...
    open(options = {}) {
      if (options.container) {
        return this.mount(options.container, options.config || options);
      }
      return this.openFullscreen(options);
    },

    /** Создание через handlers сцены (компонент / шаблон), не обычный fullscreen */
    usesSceneHandlers() {
      return !!(this._mountRoot || this._sceneConfig?.onComplete || this._sceneConfig?.sceneId);
    },

    openFullscreen(options = {}) {
      this._mountRoot = null;
      this._sceneConfig = options.config || null;
      this.resetDraft();
      ...
      document.body.classList.add('cc-fullscreen-active');
    ...
    mount(container, config = {}) {
      ...
      this.resetDraft();
      container.classList.add('cc-embedded-root');
      document.getElementById('game-content')?.classList.remove('hidden');
      document.getElementById('main')?.classList.remove('hidden');
      this.applyThemeToRoot(container);
      this.render();
    },

    unmount() {
      if (this._mountRoot) {
        this._mountRoot.innerHTML = '';
        this._mountRoot.classList.remove('cc-embedded-root');
        this._mountRoot = null;
      }
    ...
    close() {
      const root = document.getElementById('char-creator-screen');
      if (root && !this._mountRoot) {
        root.classList.add('hidden');
        root.innerHTML = '';
      }
      document.body.classList.remove('cc-fullscreen-active');
      if (!this._mountRoot) {
        document.getElementById('main')?.classList.remove('hidden');
      }
    },
    ...
    getRenderRoot() {
      return this._mountRoot || document.getElementById('char-creator-screen');
    },

    async emitSceneEvent(handlerId, draft) {
      const cfg = this._sceneConfig || {};
      const handler = handlerId || cfg.onComplete || 'char_creation_complete';
      const output = typeof CharacterCreationBridge !== 'undefined'
        ? CharacterCreationBridge.buildOutputFromDraft(GameEngine, draft)
        : { draft };
      output.draft = draft;
      const ctx = {
        sceneId: cfg.sceneId || GameEngine.state.scene,
        component: { output: { draft, character: output } },
        nextScene: cfg.nextScene
      };

      if (typeof GameEngine.runSceneHandler === 'function') {
        const res = await GameEngine.runSceneHandler(handler, ctx);
        if (res?.ok !== false) {
          CharacterCreationBridge?.ensureGameVisible?.(GameEngine);
          return true;
        }
      }

      if (typeof CharacterCreationBridge !== 'undefined') {
        CharacterCreationBridge.applyDraft(GameEngine, draft, {
          nextScene: cfg.nextScene || 'start'
        });
        return true;
      }

      GameEngine.finalizeCharacter(draft);
      return true;
    },
    ...
    getBaseAbilities(classKey) {
      ...
      return (cls.abilities || []).map((ab) => {
        if (typeof ab === 'string') {
          return pool[ab] || GameEngine.resolveAbilityDefinition?.(ab) || { id: ab, name: ab, icon: '✨' };
        }
        if (ab?.id && pool[ab.id]) return pool[ab.id];
        return ab;
      }).filter((ab) => ab && (ab.id || ab.name));
    },
    ...
    getBonusAbilityChoices(classKey) {
      ...
      const pool = GameEngine.getProgression?.().abilities || GameEngine.data?.progression?.abilities || {};
      return ids.map(id => {
        const fromPool = pool[id] || GameEngine.resolveAbilityDefinition?.(id);
        if (fromPool) return fromPool;
        const fromClass = (cls.abilities || []).find(a => typeof a === 'object' && a.id === id);
        return fromClass || { id, name: id, icon: '✨', desc: '' };
      }).filter(ab => ab && ab.id);
    },
    ...
    isFormStepValid() {
      ...
      if (!this.isWildShapeFormsComplete()) return false;
      return true;
    },
    ...
    needsWildShapeFormPick() {
      return !this.isPf2eMode() && this.draft.classKey === 'druid';
    },

    getRequiredWildShapeFormCount() {
      return typeof BeastSystem !== 'undefined' ? (BeastSystem.KNOWN_FORMS_AT_START || 2) : 2;
    },

    getWildShapeFormOptions() {
      if (typeof BeastSystem === 'undefined' || !GameEngine.data) return [];
      const unlock = BeastSystem.WILD_SHAPE_UNLOCK_LEVEL || 2;
      return BeastSystem.filterAvailableBeasts(GameEngine.data, unlock);
    },

    isWildShapeFormsComplete() {
      if (!this.needsWildShapeFormPick()) return true;
      const need = this.getRequiredWildShapeFormCount();
      return (this.draft.wildShapeKnownForms || []).length >= need;
    },

    toggleWildShapeForm(beastId) {
      if (!beastId) return;
      const need = this.getRequiredWildShapeFormCount();
      const list = [...(this.draft.wildShapeKnownForms || [])];
      const idx = list.indexOf(beastId);
      if (idx >= 0) list.splice(idx, 1);
      else if (list.length < need) list.push(beastId);
      else if (need === 1) list[0] = beastId;
      this.draft.wildShapeKnownForms = list;
      this.updateStartButton();
      this.render();
    },

    renderWildShapeFormSection() {
      if (!this.needsWildShapeFormPick()) return '';
      const need = this.getRequiredWildShapeFormCount();
      const beasts = this.getWildShapeFormOptions();
      const picked = this.draft.wildShapeKnownForms || [];
      if (!beasts.length) {
        return `<div class="cc-wild-shape-section"><p class="cc-hint">Нет зверей в данных — проверьте game_data.json → beasts.</p></div>`;
      }
      const chips = beasts.map((b) => {
        const on = picked.includes(b.id);
        const disabled = !on && picked.length >= need;
        const cr = typeof BeastSystem !== 'undefined' ? BeastSystem.formatCr(b.cr) : b.cr;
        return `<button type="button" class="cc-ability-chip cc-wild-form-chip${on ? ' active' : ''}" data-beast="${GameEngine.escapeAttr(b.id)}" ${disabled ? 'disabled' : ''} title="CR ${GameEngine.escapeAttr(cr)} · ОЗ ${b.hp} · КД ${b.ac}">
          ${GameEngine.escapeHtml(b.icon || '🐾')} ${GameEngine.escapeHtml(b.name || b.id)}
        </button>`;
      }).join('');
      return `<div class="cc-wild-shape-section">
        <h2 class="cc-section-title">Дикие облики <span class="cc-points-badge">${picked.length} / ${need}</span></h2>
        <p class="cc-hint">Выберите ${need} формы для Дикого облика (доступны с 2 уровня). Медведь — с 5 уровня.</p>
        <div class="cc-abilities-wrap">${chips}</div>
      </div>`;
    },
    ...
    render() {
      ...
      html += this.renderWildShapeFormSection();

      html += `
          <div class="form-group cc-backstory-group">
            <label for="cc-char-backstory">Предыстория</label>
            <textarea id="cc-char-backstory" class="cc-input cc-backstory" rows="5"
              placeholder="Расскажите, кем был ваш персонаж до начала приключения...">${GameEngine.escapeHtml(this.draft.backstory || '')}</textarea>
            <p class="cc-hint">Свободный текст — для себя и для сюжета. Необязательно.</p>
          </div>`;

      html += `<div class="cc-nav-row">`;
      if (this._sceneConfig?.showCancel !== false) {
        html += `<button type="button" id="cc-cancel-btn" class="choice cc-cancel-btn">← Отмена</button>`;
      }
      if (showSkillsNext) {
    ...
    bindEvents(root) {
      const cancelBtn = root.querySelector('#cc-cancel-btn');
      if (cancelBtn) {
      ...
      const backstoryInp = root.querySelector('#cc-char-backstory');
      if (backstoryInp) {
        backstoryInp.oninput = () => {
          this.draft.backstory = backstoryInp.value;
        };
      }
      ...
      root.querySelectorAll('.cc-class-card').forEach(btn => {
        btn.onclick = () => {
          ...
          this.draft.wildShapeKnownForms = [];
          const bonus = this.getBonusAbilityChoices(this.draft.classKey);
      ...
      root.querySelectorAll('.cc-wild-form-chip[data-beast]').forEach(chip => {
        chip.onclick = () => {
          this.toggleWildShapeForm(chip.getAttribute('data-beast'));
        };
      });
    ...
    async finalizeFromDraft() {
      if (!this.isValid()) return;
      const payload = { ...this.draft, wildShapeKnownForms: [...(this.draft.wildShapeKnownForms || [])] };
      if (this.isPf2eMode()) {
      ...
      if (this.usesSceneHandlers()) {
        const handler = this._sceneConfig?.onComplete || 'char_creation_complete';
        try {
          await this.emitSceneEvent(handler, payload);
        } finally {
          this._sceneConfig = null;
          this.unmount();
        }
