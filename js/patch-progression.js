  ...
  applyToGameData(data) {
    ...
    Object.values(data.classes).forEach(cls => {
      (cls.abilities || []).forEach(ab => {
        if (typeof ab === 'string') {
          const poolAb = data.progression?.abilities?.[ab];
          if (poolAb && poolAb.spellLevel == null && this.ABILITY_SPELL_LEVELS[ab] != null) {
            poolAb.spellLevel = this.ABILITY_SPELL_LEVELS[ab];
          }
          return;
        }
        if (ab && ab.spellLevel == null && this.ABILITY_SPELL_LEVELS[ab.id] != null) {
