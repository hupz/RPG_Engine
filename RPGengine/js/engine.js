...
const GameEngine = {
  ...
  /** Слот экипировки: weapon | armor | shield */
  getEquipSlot(item) {
    if (!item) return null;
    ...
    if (item.equippable && item.slot === 'accessory') return 'accessory';
    return null;
  },
  ...
  isEquippableItem(item) {
    ...
    return item.equippable && item.slot === 'accessory';
  },

  isItemEquipped(itemId) {
    return Object.values(this.state.equipped || {}).includes(itemId);
  },
  ...
  getEquippedItem(slot) {
    ...
    return id ? this.itemsData[id] : null;
  },
  ...
  getInventoryCategory(item) {
    ...
    if (t === 'weapon' || t === 'armor' || t === 'shield' || t === 'equipment') return 'equipment';
  ...
  /** Боевые статы с учётом экипировки (КД, атака, урон оружия) */
  recalculateCombatStats() {
    ...
    const stats = {
      ac: this.computePlayerAC(),
      atkBonus: prof + atkStatMod,
      dmgRoll: weaponProfile.dmgRoll,
      dmgBonus: weaponProfile.dmgBonus,
  ...
  applyLevelHpGain(level) {
    ...
    const con = this.state.classData?.stats?.con || 10;
  ...
  applyStatBonus(statsObj) {
    ...
    const stats = this.state.classData?.stats;
    ...
    for (const [key, raw] of Object.entries(statsObj)) {
      ...
      if (n <= 0) continue;
      spent += n;
      const before = stats[key] ?? 10;
      stats[key] = Math.min(20, before + n);
      parts.push(`${this.STAT_LABELS[key] || key} ${before}→${stats[key]}`);
    }
  ...
  applyPassiveAbility(ability) {
    ...
    if (passive.maxHpBonus) {
      this.state.maxHp += passive.maxHpBonus;
  ...
  unequipItem(itemIdOrSlot, opts = {}) {
    ...
    let removed = false;
    for (const slot of [...slots, 'offhand', 'accessory']) {
  ...
  useItem(itemKey) {
    ...
    if (category === 'equipment' && db.equippable && db.slot === 'accessory') {
      if (this.isItemEquipped(itemId)) this.unequipItem(itemId);
      else this.equipItem(itemId);
      return;
    }
  ...
  applyConsumableEffect(itemId) {
    ...
    const inCombat = !!(this.state.combat && this.state.combat.playerTurn);
    let spentAction = false;

    if (db.use.effect === 'heal') {
      const amount = db.use.amount != null
        ? Number(db.use.amount)
        : this.parseRoll(db.use.formula || '1d4');
      this.state.hp = Math.min(this.state.maxHp, this.state.hp + amount);
      this.log(`🧪 ${db.name}! +${amount} ОЗ`, 'log-heal');
      this.removeItem(itemId);
      if (inCombat) spentAction = true;
    } else if (db.use.effect === 'focus_potion') {
      this.applyFocusPotion(itemId, db);
      if (inCombat) spentAction = true;
    } else if (db.use.effect === 'rest_material') {
      this.log(db.use.message || 'Используйте отдых в боковой панели.', 'log-dice');
    } else if (db.use.message) {
      this.log(db.use.message, 'log-dice');
    }
  ...
  buildInvItemElement(itemId, db) {
    ...
    } else if (category === 'equipment' && db.equippable && db.slot === 'accessory') {
      if (equipped) {
        actions = `<button type="button" class="inv-btn inv-btn-unequip" ${this.onclickGame('unequipItem', itemId)}>Снять</button>`;
      } else {
        actions = `<button type="button" class="inv-btn inv-btn-equip" ${this.onclickGame('equipItem', itemId)}>Надеть</button>`;
    ...
    div.innerHTML = `
      <span class="item-name" ${nameClick} title="${this.escapeAttr(db.desc || '')}">
        ${icon}${equipTag}${this.escapeHtml(db.name)}
      </span>
  ...
  renderAbilities() {
    ...
    el.innerHTML = cd.abilities.map(ab => {
      ...
      let canUse = !isPassive && canAfford && !used;
  ...
  nextCombatTurn() {
    ...
    if (this.state.enemies.every(e => e.hp <= 0)) {
      ...
      const ca = document.getElementById('combat-area');
      if (ca) ca.classList.add('hidden');
      if (next && this.data.scenes[next]) {
        this.showScene(next);
      } else if (next) {
        this.setText('Ошибка: сцена «' + next + '» не найдена.');
        this.setChoices([]);
      } else {
        this.setChoices([]);
      }
      this.awardCombatExp(combat.enemyIds, combat.expKey);
      return;
    }
  ...
  playerCombatTurn() {
    ...
    if (this.state.inventory.includes('healing_potion')) {
      html += `<button type="button" class="choice" onclick="GameEngine.usePotionInCombat()">🧪 Выпить зелье лечения (2к4+2)</button>`;
    }
    html += `<button type="button" class="choice" onclick="GameEngine.playerFlee()">🏃 Отступить (к20+${cls.initBonus} vs DC 14)</button>`;
    area.innerHTML = html;
  },

  applyAbilityLogic(ability) {
    let endsTurn = true;
  ...
  useAbility(abilityIdOrObj) {
    ...
    const endsTurn = this.applyAbilityLogic(ability);
