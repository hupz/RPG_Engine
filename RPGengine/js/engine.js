...
const GameEngine = {
  ...
  claimFindAlbertReward() {
    ...
    if (gold > 0) {
      ...
      this.log(`💰 +${gold} зм (награда от Марты)`, 'log-heal');
    }
    (rewards.items || []).forEach((itemId) => {
      if (itemId) this.addItem(itemId);
    });
    const repFlag = rewards.reputation === 'village_hero' ? 'rep_village' : rewards.reputation;
    if (repFlag) this.changeReputation(repFlag, 15);

    this.state.flags.find_albert_rewardClaimed = true;
    this.state.flags.albertAtVillage = true;
    this.updateQuest(questId, 'complete');
    this.updateStats();
    this.saveGame();
    return true;
  },
  ...
  applyEffect(effect, target = null) {
    ...
    if (effect && typeof effect === 'object' && effect.type) {
      ...
      switch(effect.type) {
        ...
        case 'wild_shape': {
          if (this.state.combat) this.applyAcBonus(2);
          this.log('🐻 Дикий облик: +2 КД до конца боя.', 'log-combat');
          this.playCombatSound('buff');
          break;
        }
        case 'buff': {
  ...
  renderAbilities() {
    ...
    el.innerHTML = cd.abilities.map(ab => {
      ...
      const unavailable = inCombat ? this.getAbilityUnavailableReason(ab) : null;
  ...
  playerCombatTurn() {
    ...
    html += this.renderCombatActionSection('🏃 ДЕЙСТВИЯ',
      `<button type="button" class="choice" onclick="GameEngine.playerFlee()">🏃 Отступить (к20+${cls.initBonus} vs DC 14)</button>`);
