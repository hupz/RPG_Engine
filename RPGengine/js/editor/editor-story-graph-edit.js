  ...
  Object.assign(Editor, {
        ...
        <div class="story-graph-panel sg-edit-panel">
          <div class="story-graph-toolbar">
            ...
            <div class="sg-toolbar-actions">
              ...
              <button type="button" class="btn btn-secondary" onclick="Editor.createBlankScene?.(); Editor.renderStoryGraphPanel();">+ Сцена</button>
    ...
    renderEditableStoryGraph() {
      ...
      const edgesHtml = model.edges.map((e, ei) => {
        ...
        return `<g class="${cls}" data-edge-i="${ei}" data-from="${this.escapeAttr(e.fromId)}" data-ci="${e.choiceIndex}">
    ...
    deleteStoryGraphEdge(fromId, choiceIndex, toId) {
      const scene = this.data?.scenes?.[fromId];
      if (!scene) {
        alert('Сцена не найдена: ' + fromId);
        return;
      }
      ...
      if (choiceIndex < 0 || (this._sg.selectedEdge && this._sg.selectedEdge.kind === 'nextScene')) {
        if (scene.nextScene != null) {
          if (!confirm('Убрать переход «после боя» (nextScene)?')) return;
      ...
      if (idx < 0 || idx >= scene.choices.length) {
        alert('Не удалось найти выбор для удаления. Откройте сцену и удалите выбор в редакторе.');
        return;
      }
      const label = scene.choices[idx]?.text || toId || '';
      if (!confirm('Удалить связь «' + label + '»?')) return;
