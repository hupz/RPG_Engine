  ...
  Object.assign(Editor, {
    ...
    renderQuestPreview(questId) {
      ...
      const rows = keys.map(k => {
        ...
        return `<div class="quest-preview-stage"><strong>Стадия ${k}</strong> <span class="hint">${type}</span>
      ...
      return `<h4>👁 Превью «${this.escapeHtml(q.title)}»</h4>
        ...
        ${repLine}
        ${rows || '<p class="hint">Нет стадий</p>'}`;
    },
    ...
    renderQuestDetail(questId) {
      ...
      return `<div class="quest-detail-card">
        ...
        <h4>Награды</h4>
        <div class="grid-2">
          <div class="form-group"><label>Золото (зм)</label>
        ...
        <h4>Стадии</h4>
        <p class="hint">Стадия <code>0</code> — старт. В журнале — <strong>hint</strong>, в лог — <strong>log</strong> при смене. Тип «Провал» завершает квест без награды (в сценах: <code>failQuest</code> или <code>questSet.stage: "failed"</code>).</p>
    ...
    renderQuestReputationFields(questId) {
      ...
      return `<div class="quest-rep-reward">
        <p class="hint">Изменение репутации при выдаче награды квеста. Отрицательное значение — квесты на воровство и т.п.</p>
        <div class="grid-2">
          ...
          <div class="form-group"><label>Изменение репутации</label>
            <input type="number" id="quest-rep-amt-${this.escapeAttr(questId)}" value="${amount}" placeholder="напр. 15 или −10"
    ...
    renderQuestStageRow(questId, stageKey, st) {
      ...
      const qid = JSON.stringify(questId);
      const sk = JSON.stringify(stageKey);
      return `<div class="quest-stage-card">
        <div class="quest-stage-head"><strong>Стадия ${this.escapeHtml(stageKey)}</strong>
          <button type="button" class="btn-remove" onclick="${this.questAttrHandler('Editor.removeQuestStage(' + qid + ',' + sk + ')')}">×</button></div>
        <div class="form-group"><label>Тип стадии</label>
          <select onchange="${this.questAttrHandler('Editor.setQuestStageType(' + qid + ',' + sk + ',this.value)')}">
            <option value="normal"${stageType === 'normal' ? ' selected' : ''}>Обычная</option>
        ...
        <div class="form-group"><label>Текст в лог (log)</label>
          <textarea rows="2" onchange="${this.questAttrHandler('Editor.updateQuestStageField(' + qid + ',' + sk + ',\'log\',this.value)')}">${this.escapeTextarea(st.log || '')}</textarea></div>
        <div class="form-group"><label>Подсказка в журнале (hint)</label>
          <input value="${this.escapeHtml(st.hint || '')}" onchange="${this.questAttrHandler('Editor.updateQuestStageField(' + qid + ',' + sk + ',\'hint\',this.value)')}"></div>
      </div>`;
    },

    setQuestStageType(questId, stageKey, type) {
      const q = this.data.quests[questId];
      if (!q?.stages?.[stageKey]) return;
      const st = q.stages[stageKey];
    ...
    renderQuestStageSelect(questId, selectedStage, onchangeAttr) {
      ...
      const opts = keys.map(k => {
        const st = this.data.quests[questId]?.stages?.[k] || {};
        const hint = st.hint || '';
        const tag = st.failed ? ' [провал]' : (st.finish ? ' [финал]' : '');
        const sel = String(k) === String(selectedStage) ? ' selected' : '';
        return `<option value="${this.escapeAttr(k)}"${sel}>${k}${tag}: ${this.escapeHtml(hint.slice(0, 40))}</option>`;
      }).join('');
