...
(function editorClickRecovery() {
  ...
  Editor.deleteStoryGraphEdge = function (fromId, choiceIndex, toId) {
    try {
      ...
      if (choiceIndex < 0 || (this._sg?.selectedEdge && this._sg.selectedEdge.kind === 'nextScene')) {
        if (scene.nextScene != null) {
          if (!confirm('Убрать переход «после боя»?')) return;
        ...
        if (idx < 0 || idx >= scene.choices.length) {
          alert('Связь не найдена в choices. Откройте сцену и удалите выбор вручную.');
          return;
        }
      ...
      alert('Ошибка удаления связи: ' + (e.message || e));
    }
  };
  ...
  if (typeof Editor.applyClassPreset === 'function') {
    ...
    Editor.applyClassPreset = function (presetId) {
      try {
        return orig(presetId);
      } catch (e) {
        console.error(e);
        alert('Ошибка пресета: ' + (e.message || e));
      }
    };
  }
