...
(function attachEditorTemplates() {
  ...
  /** Быстрые заготовки (без SceneTemplateEngine) — из бывшего «+ Новая сцена» */
  const SIMPLE_SCENE_TEMPLATES = [
    { id: 'empty', icon: '📄', label: 'Пустая сцена' },
    { id: 'fork', icon: '🚪', label: 'Развилка' },
    { id: 'ending', icon: '🏁', label: 'Концовка' },
    { id: 'skill', icon: '🔍', label: 'Проверка навыка' }
  ];

  const SIMPLE_SCENE_TEMPLATE_IDS = new Set(SIMPLE_SCENE_TEMPLATES.map((t) => t.id));

  Object.assign(Editor, {
    _templateModalType: 'empty',
    _templatePreviewHtml: '',

    isSimpleSceneTemplate(type) {
      return SIMPLE_SCENE_TEMPLATE_IDS.has(type);
    },

    getSimpleSceneTemplates() {
      return SIMPLE_SCENE_TEMPLATES;
    },
    ...
    openCreateSceneModal() {
      ...
      if (!this._templateModalType) this._templateModalType = 'empty';
    ...
    readTemplateFormParams() {
      const type = document.getElementById('tpl-type-select')?.value || this._templateModalType;
      this._templateModalType = type;
      const id = document.getElementById('tpl-scene-id')?.value?.trim();
      const name = document.getElementById('tpl-scene-name')?.value?.trim() || id;
      ...
      if (type === 'character_creation') {
        params.introText = document.getElementById('tpl-cc-intro')?.value?.trim() || '';
        params.nextScene = document.getElementById('tpl-cc-next')?.value?.trim() || exit;
        params.startingLevel = parseInt(document.getElementById('tpl-cc-level')?.value, 10) || 1;
      }
    ...
    renderTemplateTypeFields(type) {
      ...
      if (type === 'character_creation') {
        return `<div class="form-group"><label>Вступительный текст</label>
          <textarea id="tpl-cc-intro" class="form-control" rows="3" placeholder="Создайте героя…"></textarea></div>
          <div class="form-group"><label>Сцена после создания</label>
          <select id="tpl-cc-next" class="form-control">${this.getSceneOptions(exit)}</select></div>
          <div class="form-group"><label>Стартовый уровень</label>
          <input type="number" id="tpl-cc-level" class="form-control" value="1" min="1" max="20"></div>
          <p class="hint">Компонент <code>character_creator</code> встроен в сцену. Выход «start» — первая игровая сцена.</p>`;
      }
      return '';
    },

    renderTemplateTypeSelectOptions(selectedType) {
      const simpleOpts = SIMPLE_SCENE_TEMPLATES.map((t) =>
        `<option value="${this.escapeAttr(t.id)}" ${t.id === selectedType ? 'selected' : ''}>${t.icon} ${this.escapeHtml(t.label)}</option>`
      ).join('');
      const steOpts = STE
        ? STE.listBaseTemplates().map((t) =>
            `<option value="${this.escapeAttr(t.id)}" ${t.id === selectedType ? 'selected' : ''}>${t.icon} ${this.escapeHtml(t.label)}</option>`
          ).join('')
        : '';
      return `<optgroup label="Быстрые заготовки">${simpleOpts}</optgroup>`
        + (steOpts ? `<optgroup label="Полные шаблоны (автогенерация)">${steOpts}</optgroup>` : '');
    },

    renderTemplateModalForm() {
      const body = document.getElementById('template-scene-modal-body');
      if (!body) return;
      const type = this._templateModalType || 'empty';
      const isSimple = this.isSimpleSceneTemplate(type);
      const titleEl = document.getElementById('template-scene-modal-title');
      if (titleEl) titleEl.textContent = '+ Новая сцена';

      const typeOpts = this.renderTemplateTypeSelectOptions(type);
      const exitBlock = isSimple
        ? ''
      ...
      const bgBlock = isSimple
        ? ''
        : `<div class="form-group"><label>Фон (файл)</label><input type="text" id="tpl-scene-bg" class="form-control" placeholder="shop01.png"></div>`;
      const typeFields = isSimple
        ? `<p class="hint">Минимальная заготовка: текст и выборы можно дописать в редакторе сцены после создания.</p>`
        : `<div id="tpl-type-fields">${this.renderTemplateTypeFields(type)}</div>`;
      const previewHint = isSimple
        ? '<p class="hint">Для быстрой заготовки достаточно ID и локации. Предпросмотр покажет черновик JSON.</p>'
        : '<p class="hint">Нажмите «Предпросмотр»</p>';
      ...
      body.innerHTML = `
        ...
        <h4>Параметры</h4>
        <div class="form-group"><label>ID сцены</label><input type="text" id="tpl-scene-id" class="form-control" placeholder="new_scene" value="new_${this.escapeAttr(type)}"></div>
        <div class="form-group"><label>Название (локация)</label><input type="text" id="tpl-scene-name" class="form-control" placeholder="Новая локация"></div>
        ${bgBlock}
        ${exitBlock}
        ${typeFields}
        <h4 style="margin-top:16px;">Превью</h4>
        <div id="tpl-preview-box" class="tpl-preview-box">${this._templatePreviewHtml || previewHint}</div>
    ...
    previewTemplateScene() {
      ...
      if (this.isSimpleSceneTemplate(spec.template)) {
        if (typeof this.buildSceneFromTemplate !== 'function') {
          alert('Модуль быстрых шаблонов не загружен');
          return;
        }
        const main = this.buildSceneFromTemplate(spec.template, spec.id, spec.name);
        const esc = (s) => this.escapeHtml(s);
        let html = `<p><strong>${esc(main.location)}</strong> <code>${esc(spec.id)}</code></p>`;
    ...
    commitTemplateScene() {
      ...
      if (this.isSimpleSceneTemplate(spec.template)) {
        ...
        this.data.scenes[spec.id] = this.buildSceneFromTemplate(spec.template, spec.id, spec.name || spec.id);
        ...
        setTimeout(() => document.getElementById('scene-location')?.focus(), 50);
        return;
      }
