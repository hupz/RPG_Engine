  ...
  const CharacterCreator = {
    ...
    renderClassHintPanel() {
      ...
      if (this.draft.classKey) {
        ...
        if (cls?.resource) {
          html += `<div class="cc-hint-resource">⚡ ${GameEngine.escapeHtml(cls.resource.name)}: макс. ${cls.resource.max}</div>`;
        }
        html += `</div>`;
      } else {
    ...
    getHpPreviewLabel(classKey) {
      ...
      const die = { warrior: 10, wizard: 6, paladin: 10 }[classKey] ?? 10;
      return `${die} + мод. Тел`;
    },
    ...
    getStartingSkills(classKey) {
      ...
      const classSkills = raw.map(s => this.normalizeSkillId(s)).filter(Boolean);
      const race = this.getRaceConfig();
      const raceSkills = (race?.bonusSkills || []).map(s => this.normalizeSkillId(s)).filter(Boolean);
      return [...new Set([...classSkills, ...raceSkills])];
    },
    ...
    isValid() {
      ...
      if (!this.draft.skills.length) return false;
    ...
    render() {
      ...
      const skills = this.draft.classKey ? this.getStartingSkills(this.draft.classKey) : [];
      ...
      html += `<h2 class="cc-section-title">Навыки владения</h2>`;
      if (this.hasRaces() && !this.draft.raceKey) {
        html += `<p class="cc-hint">Сначала выберите расу.</p>`;
      } else if (!this.draft.classKey) {
        html += `<p class="cc-hint">Сначала выберите класс.</p>`;
      } else if (pf2e && !this.draft.heritageId) {
        html += `<p class="cc-hint">Выберите наследие (Heritage).</p>`;
      } else if (!skills.length) {
        html += `<p class="cc-hint">Для класса не заданы startingSkills.</p>`;
      } else {
        html += `<div class="cc-skills-wrap">`;
        skills.forEach(sk => {
          const id = String(sk).toLowerCase();
          const on = this.draft.skills.includes(id) || this.draft.skills.includes(sk);
          html += `<button type="button" class="cc-skill-chip${on ? ' active' : ''}" data-skill="${GameEngine.escapeAttr(id)}">${GameEngine.escapeHtml(this.skillLabel(sk))}</button>`;
        });
        html += `</div><p class="cc-hint">Отметьте хотя бы один навык.</p>`;
      }
      ...
      html += `
          <button type="button" id="cc-start-btn" class="start-btn cc-start-btn" ${valid ? '' : 'disabled'}>
    ...
    bindEvents(root) {
      ...
      root.querySelectorAll('.cc-race-card').forEach(btn => {
        btn.onclick = () => {
          ...
          if (this.isPf2eMode()) {
            ...
            const skillOpts = this.getStartingSkills(this.draft.classKey);
            this.draft.skills = skillOpts.map(s => String(s).toLowerCase());
          }
          this.render();
        };
      });
      ...
      root.querySelectorAll('.cc-skill-chip').forEach(chip => {
        chip.onclick = () => {
          const id = chip.getAttribute('data-skill');
          const idx = this.draft.skills.indexOf(id);
          if (idx >= 0) {
            if (this.draft.skills.length <= 1) return;
      ...
      root.querySelectorAll('.cc-ability-chip').forEach(chip => {
        chip.onclick = () => {
          this.draft.bonusAbilityId = chip.getAttribute('data-ability') || '';
          root.querySelectorAll('.cc-ability-chip').forEach(c => {
            c.classList.toggle('active', c.getAttribute('data-ability') === this.draft.bonusAbilityId);
          });
          this.updateStartButton();
        };
      });
