...
(function attachEditorNewProject() {
  ...
  Object.assign(Editor, {
    ...
    confirmNewProject() {
      ...
      if (!title) {
        alert(tr('editor.newProject.enterName'));
        return;
      }
      ...
      alert('✅ ' + tr('editor.export.projectCreated', { title, system: getSystemLabel(systemId) }));
    }
  });

  const origNewProject = Editor.newProject.bind(Editor);
  Editor.newProject = function () {
    if (this.data && !confirm(tr('editor.export.newProjectConfirm'))) {
      return;
    }
    ...
    input.onchange = async (e) => {
      ...
      try {
        ...
        if (typeof this.applyLoadedProject === 'function') {
          ...
          this.renderAll();
          alert('✅ ' + tr('editor.export.loadOk', { title: this.data.meta?.title || tr('editor.newProject.noTitle') }));
        }
      } catch (err) {
        alert('❌ ' + tr('editor.export.loadFail', { message: err.message }));
      }
    };
    input.click();
  };
