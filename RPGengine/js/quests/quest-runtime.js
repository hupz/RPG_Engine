...
const QuestRuntime = {
  ...
  buildStageTasks(questId, stageIndex, savedStage) {
    ...
    return defs.map((def, i) => {
      ...
      return QuestTaskRegistry.create(merged, { questId, stageIndex });
    }).filter(Boolean);
  },
  ...
  startQuest(questId, opts = {}) {
    ...
    const store = this.ensureProgressStore();
    if (this.isCompleted(questId) || this.isFailed(questId)) return false;
  ...
  _ensureStageRuntime(questId, stageIndex) {
    ...
    if (!qp.stages[key]) {
      const tasks = this.buildStageTasks(questId, stageIndex, null);
      qp.stages[key] = { tasks: tasks.map((t) => t.serialize()) };
      return tasks;
    }
    return this.buildStageTasks(questId, stageIndex, qp.stages[key]);
  },
  ...
  handleEvent(event) {
    ...
    for (const [questId, prog] of Object.entries(store)) {
      ...
      for (const task of tasks) {
        ...
        const before = task.isCompleted();
        task.onEvent(event);
        if (task.isCompleted() && !before) {
          any = true;
          stageChanged = true;
        } else if (task.getProgress() !== (task.serialize()._progress)) {
          any = true;
        }
      }
      // persist task state
  ...
  completeQuest(questId, opts = {}) {
    ...
    if (typeof this.engine?.applyQuestNpcReputation === 'function') {
      this.engine.applyQuestFinishReputation(questId);
    }
    if (typeof this.engine?.checkAchievements === 'function') {
  ...
  setStage(questId, stageRef, opts = {}) {
    const quest = this.getQuestDef(questId);
    if (!quest) return;
    if (this.isCompleted(questId) || this.isFailed(questId)) return;
    ...
    if (stageRef === 'complete' || stageRef === '__finished__' ||
        (index != null && stages[index]?.finish && index === stages.length - 1)) {
      // If finishing stage — complete tasks of that stage first via event then complete
      this.startQuest(questId, { silentLog: true });
      if (index != null) {
    ...
    this.startQuest(questId, { stageIndex: 0, silentLog: true });
    ...
    // Emit so ManualAdvance tasks on this stage complete if content "reached" the stage
    // Actually when setStage is used from questSet it means "you are now on this stage"
    // ManualAdvance on THIS stage should NOT auto-complete on enter — the stage is the objective.
    // For migration: ManualAdvance completes when StageSet targets this stage as "done".
    // Old behavior: updateQuest TO stage means current objective is that stage.
    // So we just land on the stage.
  ...
  getJournalEntries() {
    ...
    for (const [questId, prog] of Object.entries(store)) {
      ...
      const quest = this.getQuestDef(questId);
      if (!quest) continue;
      if (quest.hidden && prog.stageIndex === 0) {
        // still show if started
      }
    ...
    // Also surface legacy-only questStages without questProgress
    if (this.state?.questStages) {
      for (const [questId, stageKey] of Object.entries(this.state.questStages)) {
        if (store[questId]) continue;
        if (stageKey === '__finished__' || stageKey === '__failed__') continue;
        const quest = this.getQuestDef(questId);
        if (!quest) continue;
        const idx = this.resolveStageIndex(quest, stageKey);
        const stage = (quest.stages || [])[idx ?? 0] || {};
        if (stage.finish || stage.failed) continue;
        out.push({
          questId,
          title: quest.title || questId,
          status: 'active',
          ...
          stageTitle: stage.title || stage.hint || '',
          hint: stage.hint || '',
          tasks: (stage.tasks || []).map((td) => ({
            id: td.id,
            text: td.description || td.type,
            done: false,
            progress: 0,
            target: td.count || 1
          }))
  ...
  /**
   * Hydrate runtime from save (questProgress + legacy questStages).
   */
  hydrateFromSave(engine) {
    this.bind(engine);
    const store = this.ensureProgressStore();
    // Migrate legacy questStages → questProgress if needed
    const qs = engine.state.questStages || {};
    for (const [questId, stageKey] of Object.entries(qs)) {
      if (store[questId]) continue;
      if (stageKey === '__finished__') {
        store[questId] = { status: 'completed', stageIndex: 0, stages: {} };
        continue;
      }
      if (stageKey === '__failed__') {
        store[questId] = { status: 'failed', stageIndex: 0, stages: {} };
        continue;
      }
      const quest = this.getQuestDef(questId);
      const idx = this.resolveStageIndex(quest, stageKey) ?? (Number(stageKey) || 0);
