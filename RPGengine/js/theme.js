...
const ThemeSystem = {
  ...
  applyChromeTokens(resolved, root = document.documentElement) {
    const isDark = this.getStoredMode() === 'dark'
      || resolved?.id === this.DARK_PRESET
      || this.luminance(resolved?.colors?.paper || '#888888') < 0.35;

    if (isDark) {
      root.style.setProperty('--editor-header-bg', '#0F3460');
      root.style.setProperty('--editor-header-fg', '#E0E0E0');
      root.style.setProperty('--editor-surface-info', 'color-mix(in srgb, var(--info) 20%, var(--card-bg))');
      root.style.setProperty('--editor-surface-warn', 'color-mix(in srgb, var(--gold) 16%, var(--card-bg))');
      root.style.setProperty('--editor-surface-success', 'color-mix(in srgb, var(--success) 16%, var(--card-bg))');
      root.style.setProperty('--editor-surface-muted', 'var(--paper-dark)');
      root.style.setProperty('--input-bg', 'var(--card-bg)');
      root.style.setProperty('--input-fg', 'var(--ink)');
    } else {
      root.style.setProperty('--editor-header-bg', '#3d5a80');
      root.style.setProperty('--editor-header-fg', '#ffffff');
      root.style.setProperty('--editor-surface-info', '#e3f2fd');
      root.style.setProperty('--editor-surface-warn', '#fff8e1');
      root.style.setProperty('--editor-surface-success', '#e8f5e9');
      root.style.setProperty('--editor-surface-muted', '#f5f0e6');
      root.style.setProperty('--input-bg', 'var(--card-bg)');
      root.style.setProperty('--input-fg', 'var(--ink)');
    }
  },
